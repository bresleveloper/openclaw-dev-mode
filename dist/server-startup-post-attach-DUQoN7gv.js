...
import { o as hasRestartSentinel } from "./restart-sentinel-CXPZNLye.js";
...
const loadMainSessionRestartRecoveryModule = createLazyRuntimeModule(() => import("./main-session-restart-recovery-DzZ3YHuH.js"));
const loadAgentDefaultsModule = createLazyRuntimeModule(() => import("./defaults-RurGC76M.js"));
const loadAgentModelSelectionModule = createLazyRuntimeModule(() => import("./model-selection-B-Q3YQMF.js"));
const loadInternalHooksModule = createLazyRuntimeModule(() => import("./internal-hooks-CHGiuwpC.js"));
const loadGatewayRestartSentinelModule = createLazyRuntimeModule(() => import("./server-restart-sentinel-tW9IIJzf.js"));
...
function scheduleGatewayMemoryBackend(params) {
	...
	const start = () => {
		import("./server-startup-memory-D5FPWDrd.js").then(({ startGatewayMemoryBackend }) => startGatewayMemoryBackend({
			cfg: params.cfg,
			log: params.log
		})).catch((err) => {
...
function scheduleProviderAuthStatePrewarm(params) {
	...
	(async () => {
		...
		const loadProviderAuthWarmModule = () => import("./model-provider-auth-DMu9fGqF.js");
...
function scheduleAgentRuntimePluginPrewarm(params) {
	...
	timer = setTimeout(() => {
		...
		measureStartup(params.startupTrace, "post-ready.agent-runtime-plugins", async () => {
			...
			const { ensureRuntimePluginsLoaded } = await import("./runtime-plugins-C9aXMdVb.js");
			const cfg = params.getConfig();
			if (isStopped()) return;
...
function scheduleTranscriptsAutoStartSidecar(params) {
	...
	return schedulePostReadySidecarTask({
		...
		log: params.log,
		run: async (isStopped) => {
			const { createTranscriptsAutoStartService } = await import("./transcripts-tool-Bt7wgWQu.js");
			if (isStopped()) return;
...
async function prewarmConfiguredPrimaryModel(params) {
	...
	const { ensureOpenClawModelsJson } = await import("./models-config-BNcdDFK1.js");
...
async function startGatewaySidecars(params) {
	...
	await measureStartup(params.startupTrace, "sidecars.internal-hooks", async () => {
		try {
			if (internalHooksConfigured) {
				const [{ setInternalHooksEnabled }, { loadInternalHooks }] = await Promise.all([loadInternalHooksModule(), import("./loader-BvJGJOfx.js")]);
	...
	let pluginServices = params.shouldStartPluginServices?.() === false ? null : await measureStartup(params.startupTrace, "sidecars.plugin-services", async () => {
		try {
			const { startPluginServices } = await import("./services-BK4gZYIN.js");
	...
	schedulePostReadySidecarTask({
		...
		log: params.log,
		run: async () => {
			const { scheduleSubagentOrphanRecovery } = await import("./subagent-registry-BMUpdRJf.js");
	...
	if (params.cfg.hooks?.enabled && params.cfg.hooks.gmail?.account) postReadySidecars.push(schedulePostReadySidecarTask({
		...
		run: async (isStopped, signal) => {
			const { startGmailWatcherWithLogs } = await import("./gmail-watcher-lifecycle-B3nYSTPz.js");
			if (isStopped()) return;
	...
	if (params.cfg.hooks?.gmail?.model) postReadySidecars.push(schedulePostReadySidecarTask({
		...
		run: async (isStopped) => {
			const [{ DEFAULT_MODEL, DEFAULT_PROVIDER }, { loadModelCatalog }, { getModelRefStatus, resolveConfiguredModelRef, resolveHooksGmailModel }] = await Promise.all([
				loadAgentDefaultsModule(),
				import("./model-catalog-B5-rgcvG.js"),
				loadAgentModelSelectionModule()
			]);
			if (isStopped()) return;
...
const defaultGatewayPostAttachRuntimeDeps = {
	...
	logGatewayStartup: async (params) => (await import("./server-startup-log-DcAByvBa.js")).logGatewayStartup(params),
	refreshLatestUpdateRestartSentinel: refreshLatestUpdateRestartSentinelIfPresent,
	scheduleGatewayUpdateCheck: async (...args) => (await import("./update-startup-5RrRNEDw.js")).scheduleGatewayUpdateCheck(...args),
	startGatewaySidecars,
	startGatewayTailscaleExposure: async (...args) => (await import("./server-tailscale-B7WEkiig.js")).startGatewayTailscaleExposure(...args)
};
...
async function startGatewayPostAttachRuntime(params, runtimeDeps = defaultGatewayPostAttachRuntimeDeps) {
	...
	sidecarsPromise.then(async (sidecarsResult) => {
		...
		if (hookRunner?.hasHooks("gateway_start")) {
			const { withPluginHttpRouteRegistry } = await import("./http-registry-iEAVug-j.js");
