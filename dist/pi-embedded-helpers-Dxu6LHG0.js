import { f as resolveSessionAgentId, i as resolveAgentConfig } from "./run-with-concurrency-C4XHHPgL.js";
import { a as resolveGatewayPort, d as resolveRequiredHomeDir, n as STATE_DIR } from "./paths-DkxwiA8g.js";
import { i as defaultRuntime, t as createSubsystemLogger, u as isDevMode } from "./subsystem-CJA8wzR-.js";
import { B as resolveAgentIdFromSessionKey, K as normalizeOptionalAccountId, N as buildAgentMainSessionKey, R as normalizeAgentId, T as resolvePathViaExistingAncestorSync, _ as runExec, a as DEFAULT_IDENTITY_FILENAME, b as openBoundaryFile, c as DEFAULT_USER_FILENAME, i as DEFAULT_HEARTBEAT_FILENAME, j as DEFAULT_AGENT_ID, l as ensureAgentWorkspace, n as DEFAULT_AGENT_WORKSPACE_DIR, o as DEFAULT_SOUL_FILENAME, r as DEFAULT_BOOTSTRAP_FILENAME, s as DEFAULT_TOOLS_FILENAME, t as DEFAULT_AGENTS_FILENAME, z as normalizeMainKey } from "./workspace-CfdKlr4m.js";
import { E as truncateUtf16Safe, h as normalizeE164, y as resolveUserPath } from "./logger-BbAT83Qh.js";
import { $ as createConfigIO, Cr as hasConfiguredSecretInput, Er as resolveSecretInputRef, J as resolveSecretRefValues, Jt as CHANNEL_IDS, St as parseByteSize, _r as formatCliCommand, bt as getBlockedNetworkModeReason, gr as isPidAlive, hr as getProcessStartTime, mr as resolveProcessScopedMap, nt as loadConfig, st as writeConfigFile, xt as parseDurationMs, yt as secretRefKey } from "./model-selection-CjLMEx1O.js";
import { c as parseBooleanValue } from "./legacy-names-DOSIC6ex.js";
import { d as getChannelDock, o as normalizeThinkLevel, v as normalizeHyphenSlug } from "./thinking-Da7Taxu9.js";
import { r as normalizeChannelId, x as normalizeChatType } from "./plugins-DOPwZyoX.js";
import { t as generateSecureToken } from "./secure-random-B_aylR4q.js";
import { i as getImageMetadata, m as normalizeMessageChannel, n as buildImageResizeSideGrid, p as listDeliverableMessageChannels, s as resizeToJpeg, t as IMAGE_REDUCE_QUALITY_STEPS } from "./image-ops-BIl9UeOZ.js";
import { A as DEFAULT_DOWNLOAD_DIR, B as DEFAULT_AI_SNAPSHOT_EFFICIENT_MAX_CHARS, C as ensureChromeExtensionRelayServer, D as resolveCdpReachabilityTimeouts, F as resolveWritablePathWithinRoot, L as normalizeBrowserFormField, M as DEFAULT_UPLOAD_DIR, N as resolveExistingPathsWithinRoot, R as isLoopbackHost, T as CDP_JSON_NEW_TIMEOUT_MS, U as DEFAULT_OPENCLAW_BROWSER_COLOR, V as DEFAULT_AI_SNAPSHOT_MAX_CHARS, W as DEFAULT_OPENCLAW_BROWSER_PROFILE_NAME, _ as withBrowserNavigationPolicy, a as resolveOpenClawUserDataDir, b as fetchOk, c as resolveBrowserExecutableForPlatform, f as normalizeCdpWsUrl, g as assertBrowserNavigationResultAllowed, h as assertBrowserNavigationAllowed, i as launchOpenClawChrome, j as DEFAULT_TRACE_DIR, l as captureScreenshot, m as InvalidBrowserNavigationUrlError, n as isChromeCdpReady, o as stopOpenClawChrome, p as snapshotAria, r as isChromeReachable, u as createTargetViaCdp, v as appendCdpPath, w as stopChromeExtensionRelayServer, y as fetchJson } from "./chrome-COZ7YCsN.js";
import { a as syncSkillsToWorkspace, l as resolveSandboxInputPath, m as sanitizeEnvVars, u as resolveSandboxPath } from "./skills-DNYmQKiw.js";
import { t as PATH_ALIAS_POLICIES } from "./path-alias-guards-DS7ydm_S.js";
import { n as extractErrorCode, r as formatErrorMessage } from "./errors-Dw6HjS62.js";
import { n as SsrFBlockedError } from "./proxy-env-B6RCnAA_.js";
import { i as saveMediaBuffer, n as ensureMediaDir } from "./store-BQSQNPdy.js";
...
import { a as resolveSessionTranscriptPathInDir, i as resolveSessionTranscriptPath, n as resolveSessionFilePath, s as resolveStorePath, t as resolveDefaultSessionStorePath } from "./paths-gKA8fewC.js";
import { t as emitSessionTranscriptUpdate } from "./transcript-events-BgGkIBR2.js";
import { t as sanitizeContentBlocksImages } from "./tool-images-Dg_vf6bo.js";
...
async function loadPwAiModule(mode) {
	try {
		return await import("./pw-ai-BdonqMxF.js");
	} catch (err) {
...
function createBrowserProfilesService(ctx) {
	...
	const deleteProfile = async (nameRaw) => {
		...
		if (name === (cfg.browser?.defaultProfile ?? "openclaw")) throw new Error(`cannot delete the default profile "${name}"; change browser.defaultProfile first`);
		let deleted = false;
		const state = ctx.state();
...
async function closePlaywrightBrowserConnection() {
	try {
		await (await import("./pw-ai-BdonqMxF.js")).closePlaywrightBrowserConnection();
	} catch {}
}
...
//#endregion
export { deliveryContextFromSession as $, compileGlobPatterns as $t, isRawApiErrorPayload as A, resolveConversationLabel as At, parseSessionThreadInfo as B, resolveBrowserControlAuth as Bt, isCloudCodeAssistFormatError as C, resolveFreshSessionTotalTokens as Ct, isFailoverErrorMessage as D, resolveExplicitAgentSessionKey as Dt, isFailoverAssistantError as E, resolveAgentMainSessionKey as Et, isTimeoutErrorMessage as F, registerBrowserRoutes as Ft, readSessionUpdatedAt as G, buildPluginToolGroups as Gt, resolveMirroredTranscriptText as H, trimToUndefined as Ht, ensureSandboxWorkspaceForSession as I, resolveBrowserConfig as It, updateLastRoute as J, mergeAlsoAllowPolicy as Jt, recordSessionMetaFromInbound as K, collectExplicitAllowlist as Kt, resolveSandboxContext as L, resolveProfile as Lt, parseImageDimensionError as M, acquireSessionWriteLock as Mt, parseImageSizeError as N, resolveSessionLockMaxHoldFromTimeout as Nt, isLikelyContextOverflowError as O, resolveMainSessionKey as Ot, sanitizeUserFacingText as P, createBrowserRouteContext as Pt, resolveCacheTtlMs as Q, resolveToolProfilePolicy as Qt, resolveSandboxRuntimeStatus as R, getBridgeAuthForPort as Rt, isBillingAssistantError as S, mergeSessionEntry as St, isContextOverflowError as T, canonicalizeMainSessionAlias as Tt, resolveAndPersistSessionFile as U, resolveSandboxConfigForAgent as Ut, appendAssistantMessageToSessionTranscript as V, resolveGatewayCredentialsFromConfig as Vt, loadSessionStore as W, applyOwnerOnlyToolPolicy as Wt, updateSessionStoreEntry as X, expandToolGroups as Xt, updateSessionStore as Y, stripPluginOnlyAllowlist as Yt, isCacheEnabled as Z, normalizeToolName as Zt, formatAssistantErrorText as _, resolveChannelResetConfig as _t, isMessagingToolDuplicateNormalized as a, resolveBootstrapTotalMaxChars as an, archiveSessionTranscripts as at, getApiErrorPayloadFingerprint as b, resolveThreadFlag as bt, extractToolCallsFromAssistant as c, extractToolCallNames as ct, downgradeOpenAIFunctionCallReasoningPairs as d, applyInputProvenanceToUserMessage as dt, matchesAnyGlobPattern as en, deliveryContextKey as et, downgradeOpenAIReasoningBlocks as f, hasInterSessionUserProvenance as ft, classifyFailoverReasonFromHttpStatus as g, evaluateSessionFreshness as gt, classifyFailoverReason as h, resolveSessionKey as ht, isMessagingToolDuplicate as i, resolveBootstrapPromptTruncationWarningMode as in, normalizeAccountId as it, isTransientHttpError as j, resolveGroupSessionKey as jt, isRateLimitAssistantError as k, deriveSessionMetaPatch as kt, extractToolResultId as l, parseInlineDirectives as lt, BILLING_ERROR_USER_MESSAGE as m, jsonUtf8Bytes as mt, validateGeminiTurns as n, ensureSessionHeader$1 as nn, normalizeDeliveryContext as nt, normalizeTextForComparison as o, sanitizeGoogleTurnOrdering as on, capArrayByJsonBytes as ot, isGoogleModelApi as p, normalizeInputProvenance as pt, resolveSessionStoreEntry as q, expandPolicyWithPluginGroups as qt, pickFallbackThinkingLevel as r, resolveBootstrapMaxChars as rn, normalizeSessionDeliveryFields as rt, sanitizeSessionMessagesImages as s, countToolResults as st, validateAnthropicTurns as t, buildBootstrapContextFiles as tn, mergeDeliveryContext as tt, sanitizeToolCallIdsForCloudCodeAssist as u, INPUT_PROVENANCE_KIND_VALUES as ut, formatBillingErrorMessage as v, resolveSessionResetPolicy as vt, isCompactionFailureError as w, setSessionRuntimeModel as wt, isAuthAssistantError as x, DEFAULT_RESET_TRIGGERS as xt, formatRawAssistantErrorForUi as y, resolveSessionResetType as yt, extractDeliveryInfo as z, ensureBrowserControlAuth as zt };
