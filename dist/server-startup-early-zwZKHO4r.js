...
const loadRemoteSkillsRuntimeModule = async () => await import("./remote-BIaR_g-_.js");
...
async function startGatewayPluginDiscovery(params) {
	...
	return await measureStartup(params.startupTrace, "runtime.early.discovery.start", async () => {
		const { startGatewayDiscovery } = await import("./server-discovery-runtime-D0NvoTgO.js");
...
async function startGatewayEarlyRuntime(params) {
	...
	if (!params.minimalTestGateway) {
		import("./context-DQIK88_0.js").then(({ ensureContextWindowCacheLoaded }) => ensureContextWindowCacheLoaded(params.cfgAtStart)).catch((err) => {
			params.log.warn(`Context-window cache warmup failed to start: ${String(err)}`);
		});
		const [{ primeRemoteSkillsCache, setSkillsRemoteRegistry }, taskRegistryMaintenance] = await measureStartup(params.startupTrace, "runtime.early.lazy-runtime-imports", () => Promise.all([loadRemoteSkillsRuntimeModule(), import("./task-registry.maintenance-D29NZ3Ei.js")]));
	...
	const skillsChangeUnsub = params.minimalTestGateway ? () => {} : await measureStartup(params.startupTrace, "runtime.early.skills-listener", async () => {
		const [{ registerSkillsChangeListener }, { refreshRemoteBinsForConnectedNodes }] = await Promise.all([import("./refresh-CZa5PdX9.js"), loadRemoteSkillsRuntimeModule()]);
	...
	const startMaintenance = async () => {
		...
		return await measureStartup(params.startupTrace, "post-ready.maintenance", async () => {
			const { startGatewayMaintenanceTimers } = await import("./server-maintenance-8A860h1X.js");
