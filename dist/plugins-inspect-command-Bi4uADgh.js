...
async function runPluginsInspectCommand(id, opts) {
	const { buildAllPluginInspectReports, buildPluginDiagnosticsReport, buildPluginInspectReport, buildPluginSnapshotReport, formatPluginCompatibilityNotice } = await import("./status-CB55zR68.js");
	const { loadInstalledPluginIndexInstallRecords } = await import("./installed-plugin-index-records-D5cTuwv4.js");
	...
	if (!targetPlugin) {
		if (id === "skill-workshop") {
			const { detectSkillWorkshopToolPolicyDiagnostic } = await import("./tool-policy-diagnostic-D0GF1eqe.js");
