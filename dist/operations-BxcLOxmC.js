...
const loadOverviewModule = async () => await import("./overview-Dk7mObF8.js");
...
async function runGatewayLifecycle(operation) {
	const lifecycle = await import("./lifecycle-NZ5Ycagz.js");
	if (operation === "start") {
...
async function runConfigSetOperation(params) {
	...
	const runConfigSet = ctx.deps?.runConfigSet ?? (async (setOpts) => {
		const { runConfigSet: importedRunConfigSet } = await import("./config-cli-Cx0tlNtn.js");
...
async function executeSetup(operation, runtime, opts) {
	...
	const result = await applyPersistentOperation({
		...
		opts,
		run: async (ctx) => {
			const applied = await (ctx.deps?.applySetup ?? (await import("./setup-apply-Cq9jp-wg.js")).applyCrestodianSetup)({
				workspace,
...
async function executeSetDefaultModel(operation, runtime, opts) {
	return await applyPersistentOperation({
		...
		run: async (ctx) => {
			...
			const { applyDefaultModelPrimaryUpdate } = await import("./shared-bL65PgfF.js");
	...
	const result = await applyPersistentOperation({
		...
		run: async (ctx) => {
			await (ctx.deps?.runPluginInstall ?? (async (spec, pluginRuntime) => {
				const { runPluginInstallCommand } = await import("./plugins-install-command-WA6SxtcH.js");
	...
	switch (operation.kind) {
		...
		case "plugin-list":
			await (opts.deps?.runPluginsList ?? (async (pluginRuntime) => {
				const { runPluginsListCommand } = await import("./plugins-list-command-9KVbZen_.js");
		...
		case "channel-list": {
			const [{ listChannelSetupPlugins }, { resolveChannelSetupEntries, shouldShowChannelInSetup }] = await Promise.all([import("./setup-registry-DXxSKV0m.js"), import("./discovery-7_MA0EoF.js")]);
			const snapshot = await readConfigFileSnapshotLazy();
			...
			const result = await applyPersistentOperation({
				...
				run: async (ctx) => {
					await (ctx.deps?.runPluginUninstall ?? (async (pluginId, pluginRuntime) => {
						const { runPluginUninstallCommand } = await import("./plugins-uninstall-command-CMkYjErM.js");
		...
		case "create-agent": {
			...
			return await applyPersistentOperation({
				...
				opts,
				run: async (ctx) => {
					await (ctx.deps?.runAgentsAdd ?? (await import("./agents.commands.add-UeyPgg0X.js")).agentsAddCommand)({
		...
		case "doctor":
			await (opts.deps?.runDoctor ?? (await import("./doctor-BRaaJHPl.js")).doctorCommand)(runtime, { nonInteractive: true });
			return { applied: false };
		case "doctor-fix": return await applyPersistentOperation({
			...
			opts,
			run: async (ctx) => {
				await (ctx.deps?.runDoctor ?? (await import("./doctor-BRaaJHPl.js")).doctorCommand)(ctx.runtime, {
					nonInteractive: true,
					repair: true,
					yes: true
				});
		...
		case "status": {
			const { statusCommand } = await import("./status.command-laIFtVhh.js");
		...
		case "open-tui": {
			...
			const result = await (opts.deps?.runTui ?? (await import("./tui-C6f_SAkz.js")).runTui)({
				local: true,
				session,
				deliver: false,
				historyLimit: 200
