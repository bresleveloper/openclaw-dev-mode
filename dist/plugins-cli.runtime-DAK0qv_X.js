...
import { g as shortenHomeInString } from "./utils-BApvfmPs.js";
...
import { n as collectConfiguredRuntimePluginIds, r as resolveConfiguredRuntimePluginInstallCandidate } from "./configured-runtime-plugin-installs-CBgkg9Cz.js";
//#region src/cli/plugins-cli.runtime.ts
function createModuleLoader(load) {
	let promise;
	return () => promise ??= load();
}
const loadPluginsConfigState = createModuleLoader(() => import("./config-state-AY--byal.js"));
const loadPluginsStatus = createModuleLoader(() => import("./status-BwJFIW6b.js"));
const loadPluginsCommandHelpers = createModuleLoader(() => import("./plugins-command-helpers-K1NWbyMH.js"));
const loadPluginsRegistryRefresh = createModuleLoader(() => import("./plugins-registry-refresh-w_2WgOyK.js"));
...
async function runPluginsEnableCommand(idInput) {
	...
	const { enableExplicitlySelectedPluginInConfig } = await import("./enable-CRGQ0-Du.js");
...
async function runPluginsDisableCommand(idInput) {
	...
	const { normalizePluginId } = await loadPluginsConfigState();
	const { buildPluginRegistrySnapshotReport } = await loadPluginsStatus();
	const { setPluginEnabledInConfig } = await import("./plugins-config-h9_qIjpX.js");
	const { refreshPluginRegistryAfterConfigMutation } = await loadPluginsRegistryRefresh();
	const snapshot = await readConfigFileSnapshot();
	const cfg = snapshot.sourceConfig ?? snapshot.config;
	const report = buildPluginRegistrySnapshotReport({ config: cfg });
...
async function runPluginsInstallAction(raw, opts) {
	await tracePluginLifecyclePhaseAsync("install command", async () => {
		const { runPluginInstallCommand } = await import("./plugins-install-command-DL-IHr-J.js");
...
async function runPluginsRegistryCommand(opts) {
	const { inspectPluginRegistry, refreshPluginRegistry } = await import("./plugin-registry-CYMdARzx.js");
...
async function runPluginMarketplaceListCommand(source, opts) {
	const { listMarketplacePlugins } = await import("./marketplace-Bm7wrCkP.js");
