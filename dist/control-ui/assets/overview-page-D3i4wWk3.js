import{n as e,r as t,t as n}from"./decorate-CUyPCN2p.js";import{a as r,f as i,g as a,h as o,m as s}from"./lit-runtime-B2f-BITn.js";import{a as c,i as l,n as u,r as d,t as f}from"./i18n-Cb2Gon67.js";import{r as p}from"./app-route-paths-Ckh-KQjG.js";import{t as m}from"./number-coercion-FQ9q6Y4E.js";import{n as ee}from"./session-display-SOXKSy_a.js";import{Cr as h,Er as g,Fr as _,Pr as v,Sr as y,Yt as b,br as x,cr as S,l as C,m as w,mr as T,n as E,p as D,pr as O,s as te,wr as ne,zn as k}from"./index-DMlQdn8s.js";import{a as A,c as j,i as M}from"./presenter-TTfHKEn1.js";import{c as N,d as P,i as F}from"./cron-BRAkk0dA.js";import{a as I,i as L,r as R,t as z}from"./provider-quota-summary--OGcm96u.js";function B(e){return e===`error`?`danger`:e===`warning`?`warn`:``}function V(e){return e in g?g[e]:g.radio}function H(e){return e.items.length===0?o:a`
    <section class="card ov-attention">
      <div class="card-title">${d(`overview.attention.title`)}</div>
      <div class="ov-attention-list">
        ${e.items.map(e=>a`
            <div class="ov-attention-item ${B(e.severity)}">
              <span class="ov-attention-icon">${V(e.icon)}</span>
  ...
  `}var U=/\d{3,}/g;function W(e){return a`${r(e.replace(/&/g,`&amp;`).replace(/</g,`&lt;`).replace(/>/g,`&gt;`).replace(U,e=>`<span class="blur-digits">${e}</span>`))}`}function G(e,t){let n=a`
  ...
  `}function J(e){if(!(e.usageResult!=null||e.sessionsResult!=null||e.skillsReport!=null))return q();let t=e.usageResult?.totals,n=S(t?.totalCost),r=T(t?.totalTokens),i=t?String(e.usageResult?.aggregates?.messages?.total??0):`0`,s=e.sessionsResult?.count??null,c=e.skillsReport?.skills??[],l=c.filter(e=>!e.disabled).length,u=c.filter(e=>e.blockedByAllowlist).length,f=c.length,p=e.cronStatus?.enabled??null,h=e.cronStatus?.nextWakeAtMs??null,g=e.cronJobs.length,_=e.cronJobs.filter(j).length,v=e.modelAuthStatus===null,y=(e.modelAuthStatus?.providers??[]).filter(L),b=K(z(y)),C=p==null?d(`common.na`):p?`${g} jobs`:d(`common.disabled`),w=_>0?a`<span class="danger">${_} failed</span>`:h?d(`overview.stats.cronNext`,{time:A(h)}):``,E=[{kind:`cost`,routeId:`usage`,label:d(`overview.cards.cost`),value:n,hint:`${r} tokens · ${i} msgs`},{kind:`sessions`,routeId:`sessions`,label:d(`overview.stats.sessions`),value:String(s??d(`common.na`)),hint:d(`overview.stats.sessionsHint`)},{kind:`skills`,routeId:`skills`,label:d(`overview.cards.skills`),value:`${l}/${f}`,hint:u>0?`${u} blocked`:`${l} active`},{kind:`cron`,routeId:`cron`,label:d(`overview.stats.cron`),value:C,hint:w}];if(b&&E.splice(1,0,b),v)E.push({kind:`auth`,routeId:`overview`,label:d(`overview.cards.modelAuth`),value:d(`common.na`),hint:``});else if(y.length>0){let e=y.filter(e=>e.status===`expired`||e.status===`missing`).length,t=y.filter(e=>e.status===`expiring`).length,n=e>0?a`<span class="danger"
          ...
          >`:t>0?a`<span class="warn"
            ...
            >`:d(`overview.cards.modelAuthOk`,{count:String(y.length)}),r=(e,t)=>{let n=m(e);if(n===void 0||t>=25)return null;let r=new Date(n);return n-Date.now()<1440*60*1e3?r.toLocaleTimeString(void 0,{hour:`numeric`,minute:`2-digit`}):r.toLocaleDateString(void 0,{month:`short`,day:`numeric`})},i=y.map(e=>{let t=[];for(let n of e.usage?.windows??[]){let e=Math.max(0,Math.min(100,Math.round(100-n.usedPercent))),i=(n.label||``).trim(),a=i?`${i} `:``,o=d(`overview.cards.modelAuthUsageLeft`,{pct:String(e)}),s=r(n.resetAt,e);t.push(s?`${a}${o} (${s})`:`${a}${o}`)}return e.expiry&&Number.isFinite(e.expiry.at)&&e.status!==`static`&&e.expiry.label&&e.expiry.label!==`unknown`&&t.push(d(`overview.cards.modelAuthExpiresIn`,{when:e.expiry.label})),t.length>0?`${e.displayName}: ${t.join(`, `)}`:null}).filter(e=>e!==null).slice(0,2).join(` · `)||d(`overview.cards.modelAuthProviders`,{count:String(y.length)});E.push({kind:`auth`,routeId:`overview`,label:d(`overview.cards.modelAuth`),value:n,hint:i})}let D=e.sessionsResult?.sessions.slice(0,5)??[];return a`
    <section class="ov-cards">${E.map(t=>G(t,e))}</section>
    ...
    ${D.length>0?a`
        ...
        `:o}
  `}function Y(e){if(e.events.length===0)return o;let t=e.events.slice(0,20);return a`
      ...
      <div class="ov-event-log-list">
        ${t.map(e=>a`
            <div class="ov-event-log-entry">
              <span class="ov-event-log-ts">${O(e.ts,void 0,``)}</span>
              <span class="ov-event-log-name">${e.event}</span>
              ${e.payload?a`<span class="ov-event-log-payload muted"
                    >${M(e.payload).slice(0,120)}</span
                  >`:o}
            </div>
          `)}
      </div>
    </details>
  `}var X=`\x1B`,re=RegExp(`${X}\\]8;;.*?${X}\\\\|${X}\\]8;;${X}\\\\`,`g`),Z=RegExp(`${X}\\[[0-9;]*m`,`g`);function ie(e){return e.replace(re,``).replace(Z,``)}function ae(e){if(e.lines.length===0)return o;let t=e.lines.slice(-50).map(e=>ie(e)).join(`
`);return a`
  ...
  `}function oe(e){let t=e.hello?.snapshot,n=t?.uptimeMs?y(t.uptimeMs):d(`common.na`),r=e.hello?.policy?.tickIntervalMs,i=r?`${(r/1e3).toFixed(r%1e3==0?0:1)}s`:d(`common.na`),o=t?.authMode===`trusted-proxy`,s=c(e.settings.locale)?e.settings.locale:u.getLocale();return a`
    <section class="grid">
      <div class="card">
        ...
        <div class="ov-access-grid" style="margin-top: 16px;">
          <label class="field ov-access-grid__full">
            ...
            <input
              ...
              @input=${t=>{let n=t.target.value;e.onConnectionChange({gatewayUrl:n,token:w(e.settings.gatewayUrl,n,e.settings.token)})}}
        ...
        <div class="stat-grid" style="margin-top: 16px;">
          ...
          <div class="stat">
            ...
            <div class="stat-value">
              ${e.lastChannelsRefresh?x(e.lastChannelsRefresh):d(`common.na`)}
            </div>
          </div>
        </div>
    ...
    ${J({usageResult:e.usageResult,sessionsResult:e.sessionsResult,skillsReport:e.skillsReport,cronJobs:e.cronJobs,cronStatus:e.cronStatus,modelAuthStatus:e.modelAuthStatus,onNavigate:e.onNavigate,canNavigate:e.canNavigate})}
    ${H({items:e.attentionItems})}
...
//# sourceMappingURL=overview-page-D3i4wWk3.js.map