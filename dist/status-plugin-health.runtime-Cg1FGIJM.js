...
import { r as listLoadedRuntimePluginIdsAcrossSurfaces, t as getActiveRuntimePluginRegistry } from "./active-runtime-registry-CPf0GXPZ.js";
import { r as resolveReadOnlyChannelPluginsForConfig } from "./read-only-Bahovy8i.js";
import { n as listPersistedRuntimeToolSchemaQuarantines } from "./tool-schema-quarantine-health-B6dFX4f2.js";
...
async function collectInstalledPluginHealthSnapshot(params) {
	const { buildPluginCompatibilityNotices, buildPluginSnapshotReport } = await import("./status-CB55zR68.js");
...
async function resolveUnregisteredMemoryEmbeddingProviders(params) {
	...
	try {
		const { collectRegisteredEmbeddingProviderIds, collectUnregisteredConfiguredMemoryEmbeddingProviders } = await import("./gateway-startup-plugin-ids-DtFLh_qj.js");
...
async function resolveEagerShouldRunPluginIds(params) {
	...
	try {
		const { loadGatewayStartupPluginPlan } = await import("./gateway-startup-plugin-ids-DtFLh_qj.js");
		const { resolvePluginActivationSourceConfig } = await import("./activation-source-config-BMLdJmI1.js");
		const { resolveGatewayStartupPluginActivationConfig } = await import("./plugin-activation-runtime-config-BwNvfUP-.js");
