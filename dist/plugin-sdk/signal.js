...
const EnvSecretRefSchema$1 = z.object({
...
const FileSecretRefSchema$1 = z.object({
...
const ExecSecretRefSchema$1 = z.object({
...
const SecretRefSchema$1 = z.discriminatedUnion("source", [
	EnvSecretRefSchema$1,
	FileSecretRefSchema$1,
	ExecSecretRefSchema$1
]);
...
const OPEN_APPEND_EXISTING_FLAGS = constants.O_RDWR | constants.O_APPEND | (SUPPORTS_NOFOLLOW$1 ? constants.O_NOFOLLOW : 0);
const OPEN_APPEND_CREATE_FLAGS = constants.O_RDWR | constants.O_APPEND | constants.O_CREAT | constants.O_EXCL | (SUPPORTS_NOFOLLOW$1 ? constants.O_NOFOLLOW : 0);
...
async function openWritableFileWithinRoot(params) {
	...
	const createFlags = params.append ? OPEN_APPEND_CREATE_FLAGS : OPEN_WRITE_CREATE_FLAGS$1;
	try {
		try {
...
async function copyFileWithinRootLegacy(params, source) {
	...
	try {
		...
		tempHandle = await fs$1.open(tempPath, OPEN_WRITE_CREATE_FLAGS$1, targetMode || 384);
		const sourceStream = source.handle.createReadStream();
...
const SecretProviderAliasString = Type.String({ pattern: SECRET_PROVIDER_ALIAS_PATTERN$1.source });
