...
const log$15 = createSubsystemLogger("huggingface-models");
...
async function discoverHuggingfaceModels(apiKey) {
	...
	try {
		...
		if (!response.ok) {
			log$15.warn(`GET /v1/models failed: HTTP ${response.status}, using static catalog`);
			return HUGGINGFACE_MODEL_CATALOG.map(buildHuggingfaceModelDefinition);
		}
		const data = (await response.json())?.data;
		if (!Array.isArray(data) || data.length === 0) {
			log$15.warn("No models in response, using static catalog");
			return HUGGINGFACE_MODEL_CATALOG.map(buildHuggingfaceModelDefinition);
		}
...
const log$14 = createSubsystemLogger("kilocode-models");
...
async function discoverKilocodeModels() {
	...
	try {
		...
		if (!Array.isArray(data.data) || data.data.length === 0) {
			log$14.warn("No models found from gateway API, using static catalog");
			return buildStaticCatalog();
		}
		const models = [];
		...
		for (const entry of data.data) {
			...
			try {
				...
				log$14.warn(`Skipping malformed model entry "${id}": ${String(e)}`);
			}
		}
...
const log$13 = createSubsystemLogger("venice-models");
...
async function discoverVeniceModels() {
	...
	try {
		...
		const data = await response.json();
		if (!Array.isArray(data.data) || data.data.length === 0) {
			log$13.warn("No models found from API, using static catalog");
			return staticVeniceModelDefinitions();
		}
		...
		if (error instanceof VeniceDiscoveryHttpError) {
			log$13.warn(`Failed to discover models: HTTP ${error.status}, using static catalog`);
			return staticVeniceModelDefinitions();
		}
...
const log$12 = createSubsystemLogger("agents/vercel-ai-gateway");
...
async function discoverVercelAiGatewayModels() {
	...
	try {
		...
		if (!response.ok) {
			log$12.warn(`Failed to discover Vercel AI Gateway models: HTTP ${response.status}`);
		...
		log$12.warn(`Failed to discover Vercel AI Gateway models: ${String(error)}`);
		return getStaticVercelAiGatewayModelCatalog();
	}
}
...
async function releaseHeldLock$1(normalizedFile) {
	const current = HELD_LOCKS$1.get(normalizedFile);
	if (!current) return;
	current.count -= 1;
...
async function acquireFileLock(filePath, options) {
	...
	const held = HELD_LOCKS$1.get(normalizedFile);
	if (held) {
		held.count += 1;
		return {
			lockPath,
			release: () => releaseHeldLock$1(normalizedFile)
		};
	}
	...
	for (let attempt = 0; attempt < attempts; attempt += 1) try {
		...
		HELD_LOCKS$1.set(normalizedFile, {
			count: 1,
			handle,
			lockPath
		});
