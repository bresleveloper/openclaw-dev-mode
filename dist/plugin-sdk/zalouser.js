...
const log$17 = createSubsystemLogger("huggingface-models");
...
async function discoverHuggingfaceModels(apiKey) {
	...
	try {
		...
		if (!response.ok) {
			log$17.warn(`GET /v1/models failed: HTTP ${response.status}, using static catalog`);
			return HUGGINGFACE_MODEL_CATALOG.map(buildHuggingfaceModelDefinition);
		}
		const data = (await response.json())?.data;
		if (!Array.isArray(data) || data.length === 0) {
			log$17.warn("No models in response, using static catalog");
			return HUGGINGFACE_MODEL_CATALOG.map(buildHuggingfaceModelDefinition);
		}
		...
		log$17.warn(`Discovery failed: ${String(error)}, using static catalog`);
		return HUGGINGFACE_MODEL_CATALOG.map(buildHuggingfaceModelDefinition);
	}
}
//#endregion
//#region src/agents/kilocode-models.ts
const log$16 = createSubsystemLogger("kilocode-models");
...
async function discoverKilocodeModels() {
	...
	try {
		...
		if (!response.ok) {
			log$16.warn(`Failed to discover models: HTTP ${response.status}, using static catalog`);
			return buildStaticCatalog();
		}
		const data = await response.json();
		if (!Array.isArray(data.data) || data.data.length === 0) {
			log$16.warn("No models found from gateway API, using static catalog");
			return buildStaticCatalog();
		}
		const models = [];
		...
		for (const entry of data.data) {
			...
			try {
				...
				log$16.warn(`Skipping malformed model entry "${id}": ${String(e)}`);
			}
		}
...
const log$15 = createSubsystemLogger("venice-models");
...
async function discoverVeniceModels() {
	...
	try {
		...
		const data = await response.json();
		if (!Array.isArray(data.data) || data.data.length === 0) {
			log$15.warn("No models found from API, using static catalog");
			return staticVeniceModelDefinitions();
		}
		...
		if (error instanceof VeniceDiscoveryHttpError) {
			log$15.warn(`Failed to discover models: HTTP ${error.status}, using static catalog`);
			return staticVeniceModelDefinitions();
		}
...
const log$14 = createSubsystemLogger("agents/vercel-ai-gateway");
...
async function discoverVercelAiGatewayModels() {
	...
	try {
		...
		if (!response.ok) {
			log$14.warn(`Failed to discover Vercel AI Gateway models: HTTP ${response.status}`);
		...
		log$14.warn(`Failed to discover Vercel AI Gateway models: ${String(error)}`);
		return getStaticVercelAiGatewayModelCatalog();
	}
}
