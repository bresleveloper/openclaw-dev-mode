...
const log$22 = createSubsystemLogger("agent-scope");
...
function resolveDefaultAgentId(cfg) {
	...
	if (defaults.length > 1 && !defaultAgentWarned) {
		...
		log$22.warn("Multiple agents marked default=true; using the first entry as default.");
	}
...
const log$19 = createSubsystemLogger("telegram/accounts");
...
function resolveDefaultTelegramAccountId(cfg) {
	...
	if (ids.length > 1 && !emittedMissingDefaultWarn) {
		...
		log$19.warn(`channels.telegram: accounts.default is missing; falling back to "${ids[0]}". ${formatSetExplicitDefaultInstruction("telegram")} to avoid routing surprises in multi-account setups.`);
	}
	return ids[0] ?? "default";
}
...
function discoverOpenClawPlugins(params) {
	...
	discoverInDirectory({
		dir: path.join(resolveConfigDir(env), "extensions"),
		origin: "global",
		ownershipUid: params.ownershipUid,
		candidates,
		diagnostics,
	...
	for (const record of registry.plugins) {
		...
		for (const raw of record.skills) {
			...
			if (!fs.existsSync(candidate)) {
				log$17.warn(`plugin skill path not found (${record.id}): ${candidate}`);
				continue;
			}
			if (!isPathInsideWithRealpath(record.rootDir, candidate, { requireRealpath: true })) {
				log$17.warn(`plugin skill path escapes plugin root (${record.id}): ${candidate}`);
				continue;
			}
...
function resolveContainedSkillPath(params) {
	...
	if (isPathInside$2(params.rootRealPath, candidateRealPath)) return candidateRealPath;
...
const log$16 = createSubsystemLogger("agents/auth-profiles");
//#endregion
...
function syncExternalCliCredentialsForProvider(store, profileId, provider, readCredentials, now) {
	...
	if ((!existingOAuth || existingOAuth.provider !== provider || existingOAuth.expires <= now || creds.expires > existingOAuth.expires) && !shallowEqualOAuthCredentials(existingOAuth, creds)) {
		...
		log$16.info(`synced ${provider} credentials from external cli`, {
			profileId,
...
function syncExternalCliCredentials(store) {
	...
	if (qwenCreds) {
		...
		if ((!existingOAuth || existingOAuth.provider !== "qwen-portal" || existingOAuth.expires <= now || qwenCreds.expires > existingOAuth.expires) && !shallowEqualOAuthCredentials(existingOAuth, qwenCreds)) {
			...
			mutated = true;
			log$16.info("synced qwen credentials from qwen cli", {
...
function warnRejectedCredentialEntries(source, rejected) {
	...
	log$16.warn("ignored invalid auth profile entries during store load", {
		source,
...
function loadAuthProfileStoreForAgent(agentDir, options) {
	...
	if (agentDir && !readOnly) {
		...
		if (mainStore && Object.keys(mainStore.profiles).length > 0) {
			...
			log$16.info("inherited auth-profiles from main agent", { agentDir });
			return mainStore;
		}
	}
	...
	if (shouldWrite && legacy !== null) {
		...
		try {
			...
			if (err?.code !== "ENOENT") log$16.warn("failed to delete legacy auth.json after migration", {
				err,
				legacyPath
			});
		}
...
function resolveConfigPathForDeps(deps) {
	...
	return resolveConfigPath$1(deps.env, resolveStateDir(deps.env, deps.homedir));
}
...
const OLLAMA_DEFAULT_BASE_URL = OLLAMA_NATIVE_BASE_URL;
...
function normalizeProviders(params) {
	...
	for (const [key, provider] of Object.entries(providers)) {
		...
		if (configuredApiKeyRef && configuredApiKeyRef.id.trim()) {
			...
			if (isNonSecretApiKeyMarker(normalizedConfiguredApiKey)) params.secretRefManagedProviders?.add(normalizedKey);
...
//#endregion
export { resolveDefaultSecretProviderAlias as $, isTrustedProxyAddress as A, consumeRootOptionToken as At, MarkdownConfigSchema as B, readLocalFileSafely as C, DEFAULT_ACCOUNT_ID as Ct, createPinnedDispatcher as D, logVerbose as Dt, closeDispatcher as E, createSubsystemLogger as Et, loadConfig as F, createAsyncLock as G, createConfigRuntimeEnv as H, projectConfigOntoRuntimeSourceSnapshot as I, listChannelDocks as J, buildWorkspaceSkillCommandSpecs as K, BlockStreamingCoalesceSchema as L, resolveSecretRefString as M, resolveStateDir as Mt, encodeJsonPointerToken as N, resolveRequiredHomeDir as Nt, resolvePinnedHostnameWithPolicy as O, shouldLogVerbose as Ot, getRuntimeConfigSourceSnapshot as P, isValidFileSecretRefId as Q, DmPolicySchema as R, SafeOpenError as S, resolveThreadSessionKeys as St, SsrFBlockedError as T, resolveThreadParentSessionKey as Tt, ensureAuthProfileStore as U, requireOpenAllowFrom as V, resolveOpenClawAgentDir as W, formatExecSecretRefIdValidationMessage as X, SECRET_PROVIDER_ALIAS_PATTERN$1 as Y, isValidExecSecretRefId as Z, rawDataToString as _, logWarn as _t, resolveModelRefFromString as a, deleteAccountFromConfigSection as at, kindFromMime as b, resolveUserPath as bt, isNonSecretApiKeyMarker as c, ENV_SECRET_REF_ID_RE as ct, parseStrictPositiveInteger as d, normalizeResolvedSecretInputString as dt, getActivePluginRegistry as et, resolveStorePath as f, normalizeSecretInputString as ft, resizeToJpeg as g, resolveAgentWorkspaceDir as gt, optimizeImageToPng as h, resolveAgentSkillsFilter as ht, resolveDefaultModelForAgent as i, formatPairingApproveHint as it, resolveClientIp as j, resolveOAuthDir as jt, isLoopbackHost as k, resolvePreferredOpenClawTmpDir as kt, listThinkingLevels as l, hasConfiguredSecretInput as lt, hasAlphaChannel as m, resolveAgentConfig as mt, buildModelAliasIndex as n, findOverlappingPluginHttpRoute as nt, normalizeProviders as o, setAccountEnabledInConfigSection as ot, convertHeicToJpeg as p, listAgentIds as pt, getChannelPlugin as q, normalizeProviderId as r, normalizePluginHttpPath as rt, resolveImplicitProviders as s, createAccountListHelpers as st, buildAllowedModelSet as t, requireActivePluginRegistry as tt, loadSessionStore as u, isValidEnvSecretRefId as ut, detectMime as v, normalizeStringEntries as vt, hasProxyEnvConfigured as w, normalizeAccountId as wt, maxBytesForKind as x, safeParseJson as xt, extensionForMime as y, isRecord$3 as yt, GroupPolicySchema as z };
