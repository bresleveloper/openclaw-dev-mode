...
const { listAccountIds: listAccountIds$4, resolveDefaultAccountId: resolveDefaultAccountId$4 } = createAccountListHelpers("slack");
const listSlackAccountIds = listAccountIds$4;
const resolveDefaultSlackAccountId = resolveDefaultAccountId$4;
...
const log$14 = createSubsystemLogger("huggingface-models");
...
async function discoverHuggingfaceModels(apiKey) {
	...
	try {
		...
		if (!response.ok) {
			log$14.warn(`GET /v1/models failed: HTTP ${response.status}, using static catalog`);
			return HUGGINGFACE_MODEL_CATALOG.map(buildHuggingfaceModelDefinition);
		}
		const data = (await response.json())?.data;
		if (!Array.isArray(data) || data.length === 0) {
			log$14.warn("No models in response, using static catalog");
			return HUGGINGFACE_MODEL_CATALOG.map(buildHuggingfaceModelDefinition);
		}
...
const log$13 = createSubsystemLogger("kilocode-models");
...
async function discoverKilocodeModels() {
	...
	try {
		...
		if (!response.ok) {
			log$13.warn(`Failed to discover models: HTTP ${response.status}, using static catalog`);
			return buildStaticCatalog();
		}
		const data = await response.json();
		if (!Array.isArray(data.data) || data.data.length === 0) {
			log$13.warn("No models found from gateway API, using static catalog");
			return buildStaticCatalog();
		}
		const models = [];
		...
		for (const entry of data.data) {
			...
			try {
				...
				log$13.warn(`Skipping malformed model entry "${id}": ${String(e)}`);
			}
		}
		...
		log$13.warn(`Discovery failed: ${String(error)}, using static catalog`);
		return buildStaticCatalog();
	}
}
//#endregion
...
const log$12 = createSubsystemLogger("venice-models");
...
async function discoverVeniceModels() {
	...
	try {
		...
		if (!response.ok) {
			log$12.warn(`Failed to discover models: HTTP ${response.status}, using static catalog`);
			return staticVeniceModelDefinitions();
		}
		const data = await response.json();
		if (!Array.isArray(data.data) || data.data.length === 0) {
			log$12.warn("No models found from API, using static catalog");
			return staticVeniceModelDefinitions();
		}
		...
		if (error instanceof VeniceDiscoveryHttpError) {
			log$12.warn(`Failed to discover models: HTTP ${error.status}, using static catalog`);
			return staticVeniceModelDefinitions();
		}
		log$12.warn(`Discovery failed: ${String(error)}, using static catalog`);
		return staticVeniceModelDefinitions();
	}
}
//#endregion
...
const log$11 = createSubsystemLogger("agents/vercel-ai-gateway");
...
async function discoverVercelAiGatewayModels() {
	...
	try {
		...
		if (!response.ok) {
			log$11.warn(`Failed to discover Vercel AI Gateway models: HTTP ${response.status}`);
		...
		log$11.warn(`Failed to discover Vercel AI Gateway models: ${String(error)}`);
		return getStaticVercelAiGatewayModelCatalog();
	}
}
