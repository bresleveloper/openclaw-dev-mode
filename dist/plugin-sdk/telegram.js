...
function resolveDefaultTelegramAccountId(cfg) {
	...
	if (ids.length > 1 && !emittedMissingDefaultWarn) {
		...
		log$9.warn(`channels.telegram: accounts.default is missing; falling back to "${ids[0]}". ${formatSetExplicitDefaultInstruction("telegram")} to avoid routing surprises in multi-account setups.`);
	}
