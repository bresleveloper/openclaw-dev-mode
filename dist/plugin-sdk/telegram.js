...
const log$23 = createSubsystemLogger("agent-scope");
...
function resolveDefaultAgentId(cfg) {
	...
	if (defaults.length > 1 && !defaultAgentWarned) {
		...
		log$23.warn("Multiple agents marked default=true; using the first entry as default.");
	}
...
const log$22 = createSubsystemLogger("telegram/accounts");
...
function resolveDefaultTelegramAccountId(cfg) {
	...
	if (ids.length > 1 && !emittedMissingDefaultWarn) {
		...
		log$22.warn(`channels.telegram: accounts.default is missing; falling back to "${ids[0]}". ${formatSetExplicitDefaultInstruction("telegram")} to avoid routing surprises in multi-account setups.`);
	}
...
new RegExp(`(^|[^a-zA-Z0-9_\\-/])([a-zA-Z0-9_.\\-./]+\\.(?:${FILE_EXTENSIONS_PATTERN}))(?=$|[^a-zA-Z0-9_\\-/])`, "gi");
new RegExp(`([^a-zA-Z0-9]|^)([A-Za-z]\\.(?:${FILE_EXTENSIONS_PATTERN}))(?=[^a-zA-Z0-9/]|$)`, "g");
//#endregion
...
path.join(DEFAULT_CRON_DIR, "jobs.json");
createSubsystemLogger("telegram/target-writeback");
createSubsystemLogger("telegram/send");
createSubsystemLogger("telegram/diagnostic");
//#endregion
