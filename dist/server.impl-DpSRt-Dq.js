...
import { C as stopDiagnosticHeartbeat, S as startDiagnosticHeartbeat } from "./diagnostic-DMzd1_zu.js";
...
import { s as setFallbackGatewayContextResolver } from "./server-plugins-Btsj9mrm.js";
...
function createLazyGatewayCronState(params) {
	...
	const load = async () => {
		...
		loading ??= import("./server-cron-BWjnn_Pb.js").then(({ buildGatewayCronService }) => {
			loaded = {
...
function getOpenAiHttpModule() {
	openAiHttpModulePromise ??= import("./openai-http-BBP4-Fqa.js");
	return openAiHttpModulePromise;
}
function getOpenResponsesHttpModule() {
	openResponsesHttpModulePromise ??= import("./openresponses-http-Sb6dVOv1.js");
...
function getSessionKillHttpModule() {
	sessionKillHttpModulePromise ??= import("./session-kill-http-69mb3cJ9.js");
	return sessionKillHttpModulePromise;
}
function getToolsInvokeHttpModule() {
	toolsInvokeHttpModulePromise ??= import("./tools-invoke-http-XAuxJRXl.js");
	...
	try {
		...
		const handleHooksRequest = async (req, res) => {
			...
			if (!loadedHooksRequestHandler) {
				const { createGatewayHooksRequestHandler } = await import("./hooks-Dcl84TCK.js");
...
function loadGatewayStartupPostAttachModule() {
	gatewayStartupPostAttachModulePromise ??= import("./server-startup-post-attach-CFfuR5ML.js");
...
function getChannelRuntime() {
	cachedChannelRuntimePromise ??= import("./runtime-channel-C43HJXGM.js").then(({ createRuntimeChannel }) => createRuntimeChannel());
	return cachedChannelRuntimePromise;
}
async function closeMcpLoopbackServerOnDemand() {
	const { closeMcpLoopbackServer } = await import("./mcp-http-CzVIUrcU.js");
...
const loadGatewayPluginBootstrapModule = async () => {
	gatewayPluginBootstrapModulePromise ??= import("./server-plugin-bootstrap-EjnFrH2j.js");
...
async function startGatewayServer(port = 18789, opts = {}) {
	...
	const loadStartupPluginsModule = () => {
		startupPluginsModulePromise ??= import("./server-startup-plugins-Ddf1JiAR.js");
	...
	const createCloseHandler = () => async (optsValue) => {
		...
		await createGatewayCloseHandler({
			...
			markMainSessionsAbortedForRestart: async ({ sessionKeys, sessionIds, activeRuns, reason, isActiveRun }) => {
				...
				const { markRestartAbortedMainSessions } = await import("./main-session-restart-recovery-DPHGdGbJ.js");
	...
	try {
		...
		const { execApprovalManager, pluginApprovalManager, extraHandlers, coreGatewayHandlers } = await startupTrace.measure("gateway.handlers", async () => {
			const [{ createGatewayAuxHandlers }, { coreGatewayHandlers: coreGatewayHandlersLocal }] = await Promise.all([import("./server-aux-handlers-BEeY9vqa.js"), import("./server-methods-BWYGZsuK.js")]);
			return {
		...
		const [{ attachGatewayWsHandlers }, { listPluginNodeCapabilities }] = await startupTrace.measure("gateway.ws-imports", () => Promise.all([import("./server-ws-runtime-BIbJ2p1p.js"), import("./route-capability-Br-EZr5i.js")]));
		...
		const loadScheduledServicesModule = () => {
			scheduledServicesModulePromise ??= import("./server-runtime-services-Cmmapk6N.js");
		...
		activateScheduledServicesWhenReady();
		const { startManagedGatewayConfigReloader } = await import("./server-reload-handlers-BlMQYGAI.js");
