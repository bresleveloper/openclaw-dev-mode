...
import { s as setFallbackGatewayContextResolver } from "./server-plugins-Cnstr-x-.js";
...
function createLazyGatewayCronState(params) {
	...
	const load = async () => {
		...
		loading ??= import("./server-cron-BF9I7vIc.js").then(({ buildGatewayCronService }) => {
			loaded = {
...
function getOpenAiHttpModule() {
	openAiHttpModulePromise ??= import("./openai-http-BZDDhNYC.js");
	return openAiHttpModulePromise;
}
function getOpenResponsesHttpModule() {
	openResponsesHttpModulePromise ??= import("./openresponses-http-Cz-uyAsp.js");
...
function getSessionKillHttpModule() {
	sessionKillHttpModulePromise ??= import("./session-kill-http-CAMLemli.js");
	return sessionKillHttpModulePromise;
}
function getToolsInvokeHttpModule() {
	toolsInvokeHttpModulePromise ??= import("./tools-invoke-http-DUEfTUWf.js");
	...
	try {
		...
		const handleHooksRequest = async (req, res) => {
			...
			if (!loadedHooksRequestHandler) {
				const { createGatewayHooksRequestHandler } = await import("./hooks-BZlANfmm.js");
...
function loadGatewayStartupPostAttachModule() {
	gatewayStartupPostAttachModulePromise ??= import("./server-startup-post-attach-DgwVRpVZ.js");
...
function getChannelRuntime() {
	cachedChannelRuntimePromise ??= import("./runtime-channel-BSch5JbO.js").then(({ createRuntimeChannel }) => createRuntimeChannel());
	return cachedChannelRuntimePromise;
}
async function closeMcpLoopbackServerOnDemand() {
	const { closeMcpLoopbackServer } = await import("./mcp-http-BLwCBD5x.js");
...
const loadGatewayPluginBootstrapModule = async () => {
	gatewayPluginBootstrapModulePromise ??= import("./server-plugin-bootstrap-K1EwUh_k.js");
...
async function startGatewayServer(port = 18789, opts = {}) {
	...
	const loadStartupPluginsModule = () => {
		startupPluginsModulePromise ??= import("./server-startup-plugins-B-Lr1LaG.js");
	...
	try {
		...
		const { execApprovalManager, pluginApprovalManager, extraHandlers, coreGatewayHandlers } = await startupTrace.measure("gateway.handlers", async () => {
			const [{ createGatewayAuxHandlers }, { coreGatewayHandlers: coreGatewayHandlersLocal }] = await Promise.all([import("./server-aux-handlers-BEeY9vqa.js"), import("./server-methods-B-TcwL1r.js")]);
			return {
		...
		const [{ attachGatewayWsHandlers }, { listPluginNodeCapabilities }] = await startupTrace.measure("gateway.ws-imports", () => Promise.all([import("./server-ws-runtime-BRuEJ6SS.js"), import("./route-capability-Br-EZr5i.js")]));
		...
		const loadScheduledServicesModule = () => {
			scheduledServicesModulePromise ??= import("./server-runtime-services-CTzxRc52.js");
		...
		activateScheduledServicesWhenReady();
		const { startManagedGatewayConfigReloader } = await import("./server-reload-handlers-idltHMvA.js");
