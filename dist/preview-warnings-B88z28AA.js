...
import { t as resolveDoctorPrimaryModelRef } from "./primary-model-ref-Dh2UBm2f.js";
...
const channelDoctorModuleLoader = createLazyImportLoader(() => import("./channel-doctor-w0eEJysz.js"));
...
async function resolveDoctorChannelPreviewConfig(params) {
	const [{ resolveCommandSecretRefsViaGateway }, { getConfiguredChannelsCommandSecretTargetIds }] = await Promise.all([import("./command-secret-gateway-CdQNjdle.js"), import("./command-secret-targets-CDWpMESA.js")]);
...
async function collectDoctorPreviewNotes(params) {
	...
	const { collectBlockedLegacyOpenAICodexProviderWarnings } = await import("./legacy-config-migrations.runtime.models-BcUEd2-B.js");
	warnings.push(...collectBlockedLegacyOpenAICodexProviderWarnings(params.cfg));
	const { collectActiveToolSchemaProjectionWarnings } = await import("./active-tool-schema-warnings-wpScZVi9.js");
	warnings.push(...collectActiveToolSchemaProjectionWarnings({
		cfg: params.cfg,
		env
	}));
	const channelPluginRuntime = await import("./channel-plugin-blockers-wbYvfa2Q.js");
	...
	if (hasChannelConfig) {
		...
		const { collectOpenPolicyAllowFromWarnings, maybeRepairOpenPolicyAllowFrom } = await import("./open-policy-allowfrom-DpHHxTku.js");
	...
	if ((hasPluginConfig || hasChannelConfig) && params.cfg.plugins?.enabled !== false) {
		const { collectStalePluginConfigWarnings, isStalePluginAutoRepairBlocked, scanStalePluginConfig } = await import("./stale-plugin-config-Dkt2zW5o.js");
	...
	if (hasPluginConfig) {
		const { collectCodexRouteWarnings } = await import("./codex-route-warnings-o0eWQhtT.js");
		warnings.push(...collectCodexRouteWarnings({
			cfg: params.cfg,
			env
		}));
		const { collectContextEngineHostCompatibilityWarnings } = await import("./context-engine-host-compat-DL5INNmU.js");
	...
	if (hasSubagentAllowlistConfig(params.cfg)) {
		const { collectStaleSubagentAllowlistWarnings, scanStaleSubagentAllowlistReferences } = await import("./stale-subagent-allowlist-BBFjQomT.js");
	...
	const { collectCodexNativeAssetInfoNotes } = await import("./codex-native-assets-C1hz-cem.js");
	infoNotes.push(...await collectCodexNativeAssetInfoNotes({
		cfg: params.cfg,
		env
	}));
	if (hasPluginLoadPaths(params.cfg)) {
		const { collectBundledPluginLoadPathWarnings, scanBundledPluginLoadPathMigrations } = await import("./bundled-plugin-load-paths-Cmbdt7Q0.js");
	...
	if (hasChannelConfig) {
		...
		const { scanEmptyAllowlistPolicyWarnings } = await import("./empty-allowlist-scan-BoXAtIhR.js");
	...
	if (hasToolsBySenderKey(params.cfg)) {
		const { collectLegacyToolsBySenderWarnings, scanLegacyToolsBySenderKeys } = await import("./legacy-tools-by-sender-Dfg_lD01.js");
	...
	if (hasConfiguredSafeBins(params.cfg)) {
		const { collectExecSafeBinCoverageWarnings, collectExecSafeBinTrustedDirHintWarnings, scanExecSafeBinCoverage, scanExecSafeBinTrustedDirHints } = await import("./exec-safe-bins-D1IHUphY.js");
	...
	const { collectStaleOAuthProfileShadowWarnings, scanStaleOAuthProfileShadows } = await import("./stale-oauth-profile-shadows-Dkg-0a9I.js");
