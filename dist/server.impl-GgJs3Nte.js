...
import { s as setFallbackGatewayContextResolver } from "./server-plugins-CSDwVzNc.js";
...
function createLazyGatewayCronState(params) {
	...
	const load = async () => {
		...
		loading ??= import("./server-cron-BppyUnGm.js").then(({ buildGatewayCronService }) => {
			loaded = {
...
function getOpenAiHttpModule() {
	openAiHttpModulePromise ??= import("./openai-http-C6IYGFcU.js");
	return openAiHttpModulePromise;
}
function getOpenResponsesHttpModule() {
	openResponsesHttpModulePromise ??= import("./openresponses-http-BB0uVjOv.js");
...
function getSessionKillHttpModule() {
	sessionKillHttpModulePromise ??= import("./session-kill-http-DIIYDyJ9.js");
	return sessionKillHttpModulePromise;
}
function getToolsInvokeHttpModule() {
	toolsInvokeHttpModulePromise ??= import("./tools-invoke-http-E_mr1wqV.js");
	...
	try {
		...
		const handleHooksRequest = async (req, res) => {
			...
			if (!loadedHooksRequestHandler) {
				const { createGatewayHooksRequestHandler } = await import("./hooks-ySt2AfA6.js");
...
function loadGatewayStartupPostAttachModule() {
	gatewayStartupPostAttachModulePromise ??= import("./server-startup-post-attach-CX5XbgBQ.js");
...
function getChannelRuntime() {
	cachedChannelRuntimePromise ??= import("./runtime-channel-C-z9r-M-.js").then(({ createRuntimeChannel }) => createRuntimeChannel());
	return cachedChannelRuntimePromise;
}
async function closeMcpLoopbackServerOnDemand() {
	const { closeMcpLoopbackServer } = await import("./mcp-http-BFsK4RsT.js");
...
const loadGatewayPluginBootstrapModule = async () => {
	gatewayPluginBootstrapModulePromise ??= import("./server-plugin-bootstrap-DGv-UfHZ.js");
...
async function startGatewayServer(port = 18789, opts = {}) {
	...
	const loadStartupPluginsModule = () => {
		startupPluginsModulePromise ??= import("./server-startup-plugins-CASokDHq.js");
	...
	try {
		...
		const { execApprovalManager, pluginApprovalManager, extraHandlers, coreGatewayHandlers } = await startupTrace.measure("gateway.handlers", async () => {
			const [{ createGatewayAuxHandlers }, { coreGatewayHandlers: coreGatewayHandlersLocal }] = await Promise.all([import("./server-aux-handlers-BEeY9vqa.js"), import("./server-methods-BJTysaI4.js")]);
			return {
		...
		const [{ attachGatewayWsHandlers }, { listPluginNodeCapabilities }] = await startupTrace.measure("gateway.ws-imports", () => Promise.all([import("./server-ws-runtime-D4s_56vF.js"), import("./route-capability-Br-EZr5i.js")]));
		...
		const loadScheduledServicesModule = () => {
			scheduledServicesModulePromise ??= import("./server-runtime-services-oU9c56AB.js");
		...
		activateScheduledServicesWhenReady();
		const { startManagedGatewayConfigReloader } = await import("./server-reload-handlers-LkiuremV.js");
