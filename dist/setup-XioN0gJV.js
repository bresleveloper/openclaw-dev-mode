...
import { p as normalizeSecretInputString } from "./types.secrets-lIZQEgMF.js";
...
import { c as formatPluginCompatibilityNotice, r as buildPluginCompatibilitySnapshotNotices } from "./status-qK7bZwFD.js";
import { a as stripPendingPluginInstallRecords, i as hasPendingPluginInstallRecords, n as commitConfigWriteWithPendingPluginInstalls, s as unchangedPendingPluginInstallRecordIds } from "./plugins-install-record-commit-DJW-YbgH.js";
import { t as resolveSetupSecretInputString } from "./setup.secret-input-CsXcEckS.js";
import { r as runSetupMigrationImport, t as detectSetupMigrationSources } from "./setup.migration-import-beIOC3ih.js";
...
function loadAuthChoiceModule() {
	authChoiceModulePromise ??= import("./auth-choice-EaEgtkhr.js");
	return authChoiceModulePromise;
}
function loadConfigLoggingModule() {
	configLoggingModulePromise ??= import("./logging-A_v9ujlt.js");
	return configLoggingModulePromise;
}
function loadModelPickerModule() {
	modelPickerModulePromise ??= import("./model-picker-Bh4xh1dY.js");
	return modelPickerModulePromise;
}
function loadOnboardConfigModule() {
	onboardConfigModulePromise ??= import("./onboard-config-5YNGQhL9.js");
...
async function resolveAuthChoiceModelSelectionPolicy(params) {
	...
	const [{ resolveManifestProviderAuthChoice }, { resolvePluginSetupProvider }] = await Promise.all([import("./provider-auth-choices-CFd_mvIz.js"), import("./setup-registry-BGclHszY.js")]);
...
async function runSetupWizard(opts, runtimeInput, prompter) {
	...
	const onboardHelpers = await import("./onboard-helpers-2C1mLZ5h.js");
	...
	if (mode === "remote") {
		const { promptRemoteGatewayConfig } = await import("./onboard-remote-BNhJwjJp.js");
	...
	if (authChoiceFromPrompt) {
		...
		({promptAuthChoiceGrouped} = await import("./auth-choice-prompt-DLhahuEi.js"));
	...
	while (true) {
		...
		if (authChoice === "custom-api-key") {
			const { promptCustomApiConfig } = await import("./onboard-custom-C1EKfEoo.js");
	...
	const { configureGatewayForSetup } = await import("./setup.gateway-config-vTbashvK.js");
	...
	if (opts.skipChannels ?? opts.skipProviders) await prompter.note(t("wizard.setup.skipChannels"), t("wizard.setup.channelsTitle"));
		...
		const { listChannelPlugins } = await import("./plugins-C0wG73MZ.js");
		const { setupChannels } = await import("./onboard-channels-B4vWwEng.js");
	...
	if (opts.skipSearch) await prompter.note(t("wizard.setup.skipSearch"), t("wizard.setup.searchTitle"));
		...
		const { setupSearch } = await import("./onboard-search-DIPMB25O.js");
	...
	if (opts.skipSkills) await prompter.note(t("wizard.setup.skipSkills"), t("wizard.setup.skillsTitle"));
		...
		const { setupSkills } = await import("./onboard-skills-BuilAKcw.js");
		nextConfig = await setupSkills(nextConfig, workspaceDir, runtime, prompter);
	}
	if (flow !== "quickstart") {
		const { setupOfficialPluginInstalls } = await import("./setup.official-plugins-BjXiNUo_.js");
		...
		const { setupPluginConfig } = await import("./setup.plugin-config-C0Ij169K.js");
	...
	if (!opts.skipHooks) {
		const { setupInternalHooks } = await import("./onboard-hooks-_ypToBEK.js");
	...
	nextConfig = await writeSetupConfigFile(nextConfig, { allowConfigSizeDrop: configResetPerformed });
	const { finalizeSetupWizard } = await import("./setup.finalize-CCTtHNd3.js");
