...
import { s as setFallbackGatewayContextResolver } from "./server-plugins-C9ffnAIC.js";
...
function createLazyGatewayCronState(params) {
	...
	const load = async () => {
		...
		loading ??= import("./server-cron-BjXTa7wo.js").then(({ buildGatewayCronService }) => {
			loaded = {
...
function getOpenAiHttpModule() {
	openAiHttpModulePromise ??= import("./openai-http-CtTOeT-j.js");
	return openAiHttpModulePromise;
}
function getOpenResponsesHttpModule() {
	openResponsesHttpModulePromise ??= import("./openresponses-http-BBFb_dFe.js");
...
function getSessionKillHttpModule() {
	sessionKillHttpModulePromise ??= import("./session-kill-http-B2Y5tE6Q.js");
	return sessionKillHttpModulePromise;
}
function getToolsInvokeHttpModule() {
	toolsInvokeHttpModulePromise ??= import("./tools-invoke-http-4ifKMsOw.js");
	...
	try {
		...
		const handleHooksRequest = async (req, res) => {
			...
			if (!loadedHooksRequestHandler) {
				const { createGatewayHooksRequestHandler } = await import("./hooks-0y_eWOpt.js");
...
function loadGatewayStartupPostAttachModule() {
	gatewayStartupPostAttachModulePromise ??= import("./server-startup-post-attach-DleAtowp.js");
...
function getChannelRuntime() {
	cachedChannelRuntimePromise ??= import("./runtime-channel-BCpILH2W.js").then(({ createRuntimeChannel }) => createRuntimeChannel());
	return cachedChannelRuntimePromise;
}
async function closeMcpLoopbackServerOnDemand() {
	const { closeMcpLoopbackServer } = await import("./mcp-http-CsU7sh6t.js");
...
const loadGatewayPluginBootstrapModule = async () => {
	gatewayPluginBootstrapModulePromise ??= import("./server-plugin-bootstrap-5lhG47yH.js");
...
async function startGatewayServer(port = 18789, opts = {}) {
	...
	const loadStartupPluginsModule = () => {
		startupPluginsModulePromise ??= import("./server-startup-plugins-DPtUb3hA.js");
	...
	try {
		...
		const { execApprovalManager, pluginApprovalManager, extraHandlers, coreGatewayHandlers } = await startupTrace.measure("gateway.handlers", async () => {
			const [{ createGatewayAuxHandlers }, { coreGatewayHandlers: coreGatewayHandlersLocal }] = await Promise.all([import("./server-aux-handlers-BEeY9vqa.js"), import("./server-methods-C_4SfzWw.js")]);
			return {
		...
		const [{ attachGatewayWsHandlers }, { listPluginNodeCapabilities }] = await startupTrace.measure("gateway.ws-imports", () => Promise.all([import("./server-ws-runtime-D4zFolyQ.js"), import("./route-capability-Br-EZr5i.js")]));
		...
		const loadScheduledServicesModule = () => {
			scheduledServicesModulePromise ??= import("./server-runtime-services-DgmGE_KP.js");
		...
		activateScheduledServicesWhenReady();
		const { startManagedGatewayConfigReloader } = await import("./server-reload-handlers-CO8tVGfa.js");
