...
import { Mo as readConfigFileSnapshot, Un as formatSessionArchiveTimestamp } from "./auth-profiles-CIqoN6FN.js";
import "./agent-scope-BAb0j1gt.js";
import { g as resolveHomeDir, m as pathExists, v as resolveUserPath, x as shortenHomePath } from "./utils-DMInpDZe.js";
...
import { n as resolveRuntimeServiceVersion } from "./version-BvA7WhZA.js";
...
import { n as isPathWithin, t as buildCleanupPlan } from "./cleanup-utils-D8J-BQ3O.js";
...
async function resolveOutputPath(params) {
	...
	if (!rawOutput) {
		...
		const canonicalCwd = await fsPromises.realpath(cwd).catch(() => cwd);
	...
	try {
		if ((await fsPromises.stat(resolved)).isDirectory()) return path.join(resolved, basename);
	} catch {}
	return resolved;
}
...
async function publishTempArchive(params) {
	try {
		await fsPromises.link(params.tempArchivePath, params.outputPath);
	} catch (err) {
		const code = err?.code;
		...
		try {
			await fsPromises.copyFile(params.tempArchivePath, params.outputPath, constants.COPYFILE_EXCL);
		} catch (copyErr) {
			const copyCode = copyErr?.code;
			if (copyCode !== "EEXIST") await fsPromises.rm(params.outputPath, { force: true }).catch(() => void 0);
			if (copyCode === "EEXIST") throw new Error(`Refusing to overwrite existing backup archive: ${params.outputPath}`, { cause: copyErr });
			throw copyErr;
		}
	}
	await fsPromises.rm(params.tempArchivePath, { force: true });
}
async function canonicalizePathForContainment(targetPath) {
	...
	let probe = resolved;
	while (true) try {
		const realProbe = await fsPromises.realpath(probe);
...
async function createBackupArchive(opts = {}) {
	...
	const tempDir = await fsPromises.mkdtemp(path.join(os.tmpdir(), "openclaw-backup-"));
	...
	try {
		...
		await fsPromises.rm(tempArchivePath, { force: true }).catch(() => void 0);
