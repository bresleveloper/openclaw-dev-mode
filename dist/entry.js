#!/usr/bin/env node
import { B as hasHelpOrVersion, H as isRootHelpInvocation, U as isRootVersionInvocation } from "./globals-DqM7Q4km.js";
	...
	for (let i = 0; i < args.length; i += 1) {
		...
		if (arg === "--dev-mode" || arg.startsWith("--dev-mode=")) {
			...
			if (value !== "0" && value !== "1") return {
				ok: false,
				error: "--dev-mode requires 0 or 1"
			};
			devMode = value;
			continue;
		}
...
if (!isMainModule({
	...
	function tryHandleRootVersionFastPath(argv) {
		...
		import("./version-cke7D5Ak.js").then((n) => n.r).then(({ VERSION }) => {
			console.log(VERSION);
		}).catch((error) => {
	...
	function tryHandleRootHelpFastPath(argv) {
		...
		import("./program-CsfrzL60.js").then(({ buildProgram }) => {
	...
	if (!ensureExperimentalWarningSuppressed()) {
		...
		if (!tryHandleRootVersionFastPath(process$1.argv) && !tryHandleRootHelpFastPath(process$1.argv)) import("./run-main-BOJ8TkFL.js").then(({ runCli }) => runCli(process$1.argv)).catch((error) => {
