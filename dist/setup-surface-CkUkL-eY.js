...
import { O as promptAccountId, Q as splitSetupEntries, v as mergeAllowFromEntries } from "./setup-wizard-helpers-Bw5FK0as.js";
...
import { r as moveSingleMatrixAccountConfigToNamedAccount, t as resolveMatrixSetupDmAllowFrom } from "./setup-dm-policy-BhInEH8O.js";
import { t as resolveMatrixConfigFieldPath } from "./config-paths-DZDSgSmi.js";
import { n as updateMatrixAccountConfig } from "./config-update-7gTv6kum.js";
import { n as validateMatrixHomeserverUrl, t as resolveValidatedMatrixHomeserverUrl } from "./url-validation-CfcV7SGb.js";
...
async function promptMatrixAllowFrom(params) {
	...
	while (true) {
		...
		if (pending.length > 0) {
			const { resolveMatrixTargets } = await import("./resolve-targets-DnHQ1TQg.js");
	...
	if (accessConfig) if (accessConfig.policy !== "allowlist") next = setMatrixGroupPolicy(next, accessConfig.policy, params.accountId);
		...
		if (accessConfig.entries.length > 0) try {
			...
			for (const entry of accessConfig.entries) {
				...
				const { listMatrixDirectoryGroupsLive } = await import("./directory-live-B0OICw0Q.js");
...
const matrixOnboardingAdapter = {
	...
	afterConfigWritten: async ({ previousCfg, cfg, accountId, runtime }) => {
		const { runMatrixSetupBootstrapAfterConfigWrite } = await import("./setup-bootstrap-DLlmMngQ.js");
