...
import { Ar as getHeadersWithAuth, Dr as normalizeCdpWsUrl, Er as formatAriaSnapshot, Jr as markPwAiLoaded, Mr as withCdpSocket, Or as appendCdpPath, Vr as getChromeWebSocketUrl, Zr as withNoProxyForCdpUrl, ci as assertBrowserNavigationAllowed, di as withBrowserNavigationPolicy, ii as DEFAULT_UPLOAD_DIR, jr as normalizeCdpHttpBaseForJsonEndpoints, kr as fetchJson, li as assertBrowserNavigationRedirectChainAllowed, oi as resolveStrictExistingPathsWithinRoot, ri as DEFAULT_TRACE_DIR, si as BrowserTabNotFoundError, ui as assertBrowserNavigationResultAllowed } from "./auth-profiles-CIqoN6FN.js";
...
import { S as writeFileFromPathWithinRoot } from "./windows-spawn-D2jCT_tt.js";
...
async function writeViaSiblingTempPath(params) {
	const rootDir = await fsPromises.realpath(path.resolve(params.rootDir)).catch(() => path.resolve(params.rootDir));
	const requestedTargetPath = path.resolve(params.targetPath);
	const targetPath = await fsPromises.realpath(path.dirname(requestedTargetPath)).then((realDir) => path.join(realDir, path.basename(requestedTargetPath))).catch(() => requestedTargetPath);
