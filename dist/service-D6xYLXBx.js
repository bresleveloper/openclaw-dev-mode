import { i as parseStrictPositiveInteger, n as parseStrictInteger } from "./parse-finite-number-CtD2ba8J.js";
import { f as resolveGatewayServiceDescription, h as resolveLegacyGatewayLaunchAgentLabels, m as resolveGatewayWindowsTaskName, n as GATEWAY_LAUNCH_AGENT_LABEL, u as resolveGatewayLaunchAgentLabel } from "./arg-split-Co_4JYiE.js";
import { a as renderCmdSetAssignment, i as parseCmdSetAssignment, n as quoteCmdScriptArg, r as assertNoCmdLineBreak, t as parseCmdScriptCommandLine } from "./cmd-argv-CCH9iAjU.js";
...
async function ensureSecureDirectory(targetPath) {
	...
	try {
		const mode = (await fsPromises.stat(targetPath)).mode & 511;
		const tightenedMode = mode & -19;
		if (tightenedMode !== mode) await fsPromises.chmod(targetPath, tightenedMode);
	} catch {}
}
...
async function installLaunchAgent({ env, stdout, programArguments, workingDirectory, environment, description }) {
	...
	await fsPromises.writeFile(plistPath, plist, {
		encoding: "utf8",
		mode: LAUNCH_AGENT_PLIST_MODE
	});
	await fsPromises.chmod(plistPath, LAUNCH_AGENT_PLIST_MODE).catch(() => void 0);
	await execLaunchctl([
		"bootout",
		domain,
		plistPath
