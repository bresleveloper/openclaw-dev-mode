...
import { n as info, s as success, t as danger } from "./globals-BWmHKFNH.js";
...
import { l as renderQrPngDataUrl } from "./media-runtime-BGWQwoXi.js";
...
import { d as readWebAuthExistsForDecision, h as readWebSelfId, n as WHATSAPP_AUTH_UNSTABLE_CODE, s as logoutWeb } from "./auth-store-BERBvAOr.js";
import { t as createWaSocket } from "./session-CjoWbkaw.js";
import { t as getActiveWebListener } from "./active-listener-bFJiCLwz.js";
import { i as closeWaSocket, o as waitForWhatsAppLoginResult, t as WHATSAPP_LOGGED_OUT_QR_MESSAGE } from "./connection-controller-7zCU-f--.js";
...
async function waitForQrOrRecoveredLogin(params) {
	...
	const qrUpdateResult = params.login.qrUpdatePromise.then(() => {
		const current = activeLogins.get(params.accountId);
		if (current?.id !== params.login.id) return {
			outcome: "failed",
			message: "WhatsApp login was replaced by a newer request."
		};
		...
		if (current.connected) return { outcome: "connected" };
		return {
			outcome: "failed",
			message: current.error ? `WhatsApp login failed: ${current.error}` : "WhatsApp QR update ended without an active QR."
		};
	});
...
async function startWebLoginWithQr(opts = {}) {
	...
	if (authState.exists && opts.force) try {
		if (!await logoutWeb({
