...
import { i as buildAgentMainSessionKey, u as normalizeAgentId } from "./session-key-pTKRJb0m.js";
...
const loadOverviewModule = async () => await import("./overview-zfATZ9DB.js");
const loadModelsSharedModule = async () => await import("./shared-ySBtuGac.js");
const loadConfigCliModule = async () => await import("./config-cli-BMzX_BCr.js");
const loadDoctorModule = async () => await import("./doctor-tZG0ERqp.js");
...
async function runGatewayLifecycle(operation) {
	const lifecycle = await import("./lifecycle-CHElWJqC.js");
	if (operation === "start") {
...
async function executeCrestodianOperation(operation, runtime, opts = {}) {
	...
	if (operation.kind === "plugin-search") {
		...
		await (opts.deps?.runPluginsSearch ?? (async (query, pluginRuntime) => {
			const { runPluginsSearchCommand } = await import("./plugins-search-command-vurUQ8x8.js");
	...
	if (operation.kind === "plugin-uninstall") {
		...
		await (opts.deps?.runPluginUninstall ?? (async (pluginId, pluginRuntime) => {
			const { runPluginUninstallCommand } = await import("./plugins-uninstall-command-DA2zJYw7.js");
	...
	if (operation.kind === "create-agent") {
		...
		const before = await readConfigFileSnapshot();
		const workspace = resolveUserPath(operation.workspace ?? process.cwd());
		await (opts.deps?.runAgentsAdd ?? (await import("./agents.commands.add-B2R788RX.js")).agentsAddCommand)({
	...
	if (operation.kind === "status") {
		...
		const { statusCommand } = await import("./status.command-DG21mjqA.js");
	...
	if (operation.kind === "open-tui") {
		...
		const result = await (opts.deps?.runTui ?? (await import("./tui-pDt6XyfZ.js")).runTui)({
			local: true,
			session,
			deliver: false,
			historyLimit: 200
