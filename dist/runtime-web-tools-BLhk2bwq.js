import { Vi as createGatewayCredentialPlan, au as normalizeSecretInput, co as isNonEmptyString, lo as isRecord$1, oo as resolveSecretRefValues, uo as parseDotPath } from "./auth-profiles-CIqoN6FN.js";
import { d as resolveSecretInputRef, i as coerceSecretRef } from "./types.secrets-BEA4gMCN.js";
import { jt as secretRefKey } from "./plugins-CjLMQbHJ.js";
...
function traverseToLeafParent(params) {
	...
	for (let index = 0; index < params.segments.length - 1; index += 1) {
		...
		if (!isRecord$1(cursor)) throw new Error(`Invalid path shape at ${params.segments.slice(0, index).join(".") || "<root>"}.`);
...
function setPathCreateStrict(root, segments, value) {
	...
	for (let index = 0; index < segments.length - 1; index += 1) {
		...
		if (Array.isArray(cursor)) {
			...
			if (existing === void 0 || existing === null) {
				...
				changed = true;
			} else if (needs === "array" ? !Array.isArray(existing) : !isRecord$1(existing)) throw new Error(`Invalid path shape at ${segments.slice(0, index + 1).join(".")}.`);
			cursor = cursor[arrayIndex];
			continue;
		}
		if (!isRecord$1(cursor)) throw new Error(`Invalid path shape at ${segments.slice(0, index).join(".") || "<root>"}.`);
	...
	if (!isRecord$1(cursor)) throw new Error(`Invalid path shape at ${segments.slice(0, -1).join(".") || "<root>"}.`);
...
function collectTtsApiKeyAssignments(params) {
	...
	if (isRecord$1(elevenlabs)) collectSecretInputAssignment({
	...
	if (isRecord$1(openai)) collectSecretInputAssignment({
		value: openai.apiKey,
...
function collectDiscordAssignments(params) {
	...
	if (isRecord$1(discord.pluralkit)) {
	...
	if (isRecord$1(discord.voice) && isRecord$1(discord.voice.tts)) collectTtsApiKeyAssignments({
	...
	if (!surface.hasExplicitAccounts) return;
	for (const { accountId, account, enabled } of surface.accounts) {
		if (hasOwnProperty(account, "pluralkit") && isRecord$1(account.pluralkit)) {
		...
		if (hasOwnProperty(account, "voice") && isRecord$1(account.voice) && isRecord$1(account.voice.tts)) collectTtsApiKeyAssignments({
...
function collectIrcAssignments(params) {
	...
	if (!surface.hasExplicitAccounts) return;
	for (const { accountId, account, enabled } of surface.accounts) if (hasOwnProperty(account, "nickserv") && isRecord$1(account.nickserv)) {
...
function collectBlueBubblesAssignments(params) {
	...
	if (!isRecord$1(bluebubbles)) return;
	collectSimpleChannelFieldAssignments({
...
function collectMattermostAssignments(params) {
	...
	if (!isRecord$1(mattermost)) return;
	collectSimpleChannelFieldAssignments({
...
function collectNextcloudTalkAssignments(params) {
	...
	if (!isRecord$1(nextcloudTalk)) return;
...
function resolvePerplexityRuntimeTransport(params) {
	const config = isRecord(params.configValue) ? params.configValue : void 0;
	...
	const baseUrl = (() => {
		...
		if ((params.keySource === "config" || params.keySource === "secretRef") && params.keyValue) return inferPerplexityBaseUrlFromApiKey(params.keyValue) === "openrouter" ? DEFAULT_PERPLEXITY_BASE_URL : PERPLEXITY_DIRECT_BASE_URL;
...
function envVarsForProvider(provider) {
...
function resolveProviderKeyValue(search, provider) {
	if (provider === "brave") return search.apiKey;
...
async function resolveRuntimeWebTools(params) {
	...
	if (searchEnabled && search) {
		...
		for (const provider of candidates) {
			const path = provider === "brave" ? "tools.web.search.apiKey" : `tools.web.search.${provider}.apiKey`;
		...
		if (selectedProvider) {
			...
			if (selectedProvider === "perplexity") searchMetadata.perplexityTransport = resolvePerplexityRuntimeTransport({
	...
	if (searchEnabled && search && !configuredProvider && searchMetadata.selectedProvider) for (const provider of WEB_SEARCH_PROVIDERS) {
		if (provider === searchMetadata.selectedProvider) continue;
		const path = provider === "brave" ? "tools.web.search.apiKey" : `tools.web.search.${provider}.apiKey`;
		if (!hasConfiguredSecretRef(resolveProviderKeyValue(search, provider), defaults)) continue;
		pushInactiveSurfaceWarning({
			context: params.context,
			path,
	...
	else if (search && !searchEnabled) for (const provider of WEB_SEARCH_PROVIDERS) {
		const path = provider === "brave" ? "tools.web.search.apiKey" : `tools.web.search.${provider}.apiKey`;
		if (!hasConfiguredSecretRef(resolveProviderKeyValue(search, provider), defaults)) continue;
		pushInactiveSurfaceWarning({
			context: params.context,
	...
	if (searchEnabled && search && configuredProvider) for (const provider of WEB_SEARCH_PROVIDERS) {
...
//#endregion
export { getPath as C, deletePathStrict as S, setPathExistingStrict as T, listSecretTargetRegistryEntries as _, applyResolvedAssignments as a, hasConfiguredPlaintextSecretValue as b, pushWarning as c, discoverAuthProfileSecretTargets as d, discoverConfigSecretTargets as f, listAuthProfileSecretTargetEntries as g, isKnownSecretTargetType as h, evaluateGatewayAuthSurfaceStates as i, analyzeCommandSecretAssignmentsFromSnapshot as l, isKnownSecretTargetId as m, collectConfigAssignments as n, createResolverContext as o, discoverConfigSecretTargetsByIds as p, GATEWAY_AUTH_SURFACE_PATHS as r, pushAssignment as s, resolveRuntimeWebTools as t, collectCommandSecretAssignmentsFromSnapshot as u, resolvePlanTargetAgainstRegistry as v, setPathCreateStrict as w, isExpectedResolvedSecretValue as x, assertExpectedResolvedSecretValue as y };
