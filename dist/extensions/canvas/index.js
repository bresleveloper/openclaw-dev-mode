...
import { n as isCanvasHostEnabled, t as canvasConfigSchema } from "../../config-LihfRCxb.js";
import { n as CANVAS_HOST_PATH, r as CANVAS_WS_PATH, t as A2UI_PATH } from "../../a2ui-shared-BOFwFS5N.js";
import { t as CanvasToolSchema } from "../../tool-schema-CFGO4x4D.js";
...
function createLazyCanvasTool(params) {
	...
	const loadTool = async () => {
		toolPromise ??= import("../../tool-s6AzivTH.js").then(({ createCanvasTool }) => createCanvasTool({
	...
	register(api) {
		if (isCanvasHostEnabled(api.config)) {
			...
			const loadHttpRouteHandler = async () => {
				httpRouteHandlerPromise ??= import("../../http-route-DzFZ92EY.js").then(({ createCanvasHttpRouteHandler }) => createCanvasHttpRouteHandler({
					config: api.config,
			...
			api.registerHostedMediaResolver(async (mediaUrl) => {
				resolveCanvasHttpPathToLocalPathPromise ??= import("../../documents-CrtCVx8N.js").then(({ resolveCanvasHttpPathToLocalPath }) => resolveCanvasHttpPathToLocalPath);
		...
		api.registerNodeCliFeature(async ({ program }) => {
			const { createDefaultCanvasCliDependencies, registerNodesCanvasCommands } = await import("../../cli-BKoivvTS.js");
