...
import { t as registerShortTermPromotionDreaming } from "../../dreaming-Btpqi3NP.js";
...
function loadMemoryToolsModule() {
	memoryToolsModulePromise ??= import("../../tools-XQguIRws.js");
	return memoryToolsModulePromise;
}
function loadRuntimeProviderModule() {
	runtimeProviderModulePromise ??= import("../../runtime-provider-Bo2-_AYO.js");
	...
	register(api) {
		...
		api.registerMemoryCapability({
			...
			publicArtifacts: { async listArtifacts(params) {
				const { listMemoryCorePublicArtifacts } = await import("../../public-artifacts-D_H2UZPa.js");
		...
		api.registerCommand({
			...
			acceptsArgs: true,
			handler: async (ctx) => {
				const { handleDreamingCommand } = await import("../../dreaming-command-D8UC67xB.js");
