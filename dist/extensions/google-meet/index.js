...
import { a as optionalPositiveIntegerSchema } from "../../typebox-B_TbP7xZ.js";
...
async function startCommandRealtimeAudioBridge(params) {
	...
	const startHumanBargeInMonitor = () => {
		...
		bargeInInputProcess.stdout?.on("error", (error) => {
			params.logger.warn(`[google-meet] human barge-in input stdout failed: ${formatErrorMessage$1(error)}`);
		});
		...
		bargeInInputProcess.stderr?.on("error", (error) => {
			params.logger.warn(`[google-meet] human barge-in input stderr failed: ${formatErrorMessage$1(error)}`);
		});
...
function createVoiceCallGateway(params) {
	if (!params.config.voiceCall.gatewayUrl) return {
		trustedPluginIdentity: true,
		request: (method, requestParams) => params.runtime.gateway.request(method, requestParams, { timeoutMs: params.config.voiceCall.requestTimeoutMs })
	};
	return {
		...
		async request(method, requestParams) {
			...
			try {
				return await client.request(method, requestParams, { timeoutMs: params.config.voiceCall.requestTimeoutMs });
			} finally {
...
async function joinMeetViaVoiceCallGateway(params) {
	if (params.agentId && params.agentId !== "main" && !params.gateway.trustedPluginIdentity) throw new Error("Per-agent Voice Call routing requires the local Gateway runtime. Remove google-meet voiceCall.gatewayUrl or omit agent routing.");
		...
		try {
			if (transport === "chrome" || transport === "chrome-node") {
				...
				const voiceCallResult = this.params.config.voiceCall.enabled ? await joinMeetViaVoiceCallGateway({
					...
					agentId: request.agentId,
					sessionKey: request.agentId ? `agent:${request.agentId}:google-meet:${session.id}` : buildTwilioVoiceCallSessionKey(session.id),
...
const loadGoogleMeetCreateModule = createLazyRuntimeModule(() => import("../../create-BkVt3zwG.js"));
const loadGoogleMeetCliModule = createLazyRuntimeModule(() => import("../../cli-CaLAvcvD.js"));
...
async function callGoogleMeetGatewayFromTool(params) {
	try {
		if (params.runtime) return await params.runtime.gateway.request(googleMeetGatewayMethodForToolAction(params.action), params.raw, { timeoutMs: resolveGoogleMeetGatewayOperationTimeoutMs(params.config) });
...
async function resolveGoogleMeetTokenFromParams(config, raw) {
	const { resolveGoogleMeetAccessToken } = await import("../../oauth-CICRn5Na.js");
	...
	register(api) {
		...
		api.registerTool((toolContext) => ({
			...
			async execute(_toolCallId, params) {
				...
				const agentId = toolContext.agentId ? normalizeAgentId(toolContext.agentId) : void 0;
				try {
					const useTrustedRuntime = agentId ? await api.runtime.gateway.isAvailable() : false;
					if (agentId && agentId !== "main" && !useTrustedRuntime) throw new Error("Per-agent Google Meet routing requires a Gateway-hosted agent run.");
