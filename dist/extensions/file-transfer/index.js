...
import { a as DIR_LIST_TOOL_DESCRIPTOR, c as FILE_FETCH_TOOL_DESCRIPTOR, d as FILE_WRITE_TOOL_DESCRIPTOR, r as DIR_FETCH_TOOL_DESCRIPTOR } from "../../descriptors-BxYUDWd7.js";
//#region extensions/file-transfer/src/shared/lazy-node-invoke-policy.ts
const loadFileTransferNodeInvokePolicy = async () => {
	const { createFileTransferNodeInvokePolicy } = await import("../../node-invoke-policy-x3QxVceZ.js");
	...
	nodeHostCommands: [
		{
			...
			handle: async (paramsJSON) => {
				const { handleFileFetch } = await import("../../file-fetch-DOW88Yte.js");
		...
		},
		{
			...
			dangerous: true,
			handle: async (paramsJSON) => {
				const { handleDirList } = await import("../../dir-list-C0Lyw7tj.js");
		...
		},
		{
			...
			dangerous: true,
			handle: async (paramsJSON) => {
				const { handleDirFetch } = await import("../../dir-fetch-BJ4paACO.js");
		...
		},
		{
			...
			dangerous: true,
			handle: async (paramsJSON) => {
				const { handleFileWrite } = await import("../../file-write-CAn2k0XW.js");
