...
const decoders = {
	async oga(buf) {
		...
		if (!decoder) {
			let { OggVorbisDecoder } = await import("./ogg-vorbis-B6NtpQQH.js");
	...
	async mp3(buf) {
		...
		if (!decoder) {
			const { MPEGDecoder } = await import("./mpg123-decoder-Cy6egvXa.js");
	...
	async flac(buf) {
		...
		if (!decoder) {
			const { FLACDecoder } = await import("./flac-CkjCUyZH.js");
	...
	async opus(buf) {
		...
		if (!decoder) {
			const { OggOpusDecoder } = await import("./ogg-opus-decoder-BvRQ08KK.js");
	...
	async wav(buf) {
		...
		if (!decode) {
			let module = await import("./node-wav-jwZ6Paty.js").then((m) => /* @__PURE__ */ __toESM(m.default, 1));
	...
	async qoa(buf) {
		...
		if (!decode) decoders.qoa.decode = decode = (await import("./qoa-format-CadSqeKv.js")).decode;
		return buf && createBuffer(await decode(buf));
	}
};
