...
async function runOnboardingWizard(opts, runtime = defaultRuntime, prompter) {
	...
	const { applyAuthChoice, resolvePreferredProviderForAuthChoice, warnIfModelConfigLooksOff } = await import("./auth-choice-DbUBVgyq.js").then((n) => n.t);
	...
	if (opts.skipChannels ?? opts.skipProviders) await prompter.note("Skipping channel setup.", "Channels");
		...
		const { setupChannels } = await import("./onboard-channels-DgT9ZhJE.js").then((n) => n.n);
	...
	await writeConfigFile(nextConfig);
	const { finalizeOnboardingWizard } = await import("./onboarding.finalize-BnkISaKj.js");
