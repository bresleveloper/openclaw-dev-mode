...
function createCodexAppServerAgentHarness(options) {
	...
	return {
		...
		runAttempt: async (params) => {
			const { runCodexAppServerAttempt } = await import("./run-attempt-DU4Aecso.js");
		...
		runSideQuestion: async (params) => {
			const { runCodexAppServerSideQuestion } = await import("./side-question-ZFbVpnRC.js");
		...
		compact: async (params) => {
			const { maybeCompactCodexAppServerSession } = await import("./compact-B8sMrZ3R.js");
		...
		reset: async (params) => {
			if (params.sessionFile) {
				const { clearCodexAppServerBinding } = await import("./session-binding-B9536fkF.js");
				await clearCodexAppServerBinding(params.sessionFile);
			}
		},
		dispose: async () => {
			const { clearSharedCodexAppServerClientAndWait } = await import("./shared-client-lSBL9z1S.js");
