...
import { s as setFallbackGatewayContextResolver } from "./server-plugins-BWOY_UxK.js";
...
function createLazyGatewayCronState(params) {
	...
	const load = async () => {
		...
		loading ??= import("./server-cron-CaTQ5ccv.js").then(({ buildGatewayCronService }) => {
			loaded = {
...
function getOpenAiHttpModule() {
	openAiHttpModulePromise ??= import("./openai-http-BGrM74ue.js");
	return openAiHttpModulePromise;
}
function getOpenResponsesHttpModule() {
	openResponsesHttpModulePromise ??= import("./openresponses-http-Csf9TuYm.js");
...
function getSessionKillHttpModule() {
	sessionKillHttpModulePromise ??= import("./session-kill-http-DBAvbOmy.js");
	return sessionKillHttpModulePromise;
}
function getToolsInvokeHttpModule() {
	toolsInvokeHttpModulePromise ??= import("./tools-invoke-http-BxIvQn6K.js");
	...
	try {
		...
		const handleHooksRequest = async (req, res) => {
			...
			if (!loadedHooksRequestHandler) {
				const { createGatewayHooksRequestHandler } = await import("./hooks-CaY-fL0f.js");
...
function loadGatewayStartupPostAttachModule() {
	gatewayStartupPostAttachModulePromise ??= import("./server-startup-post-attach-CBHQ20OE.js");
...
function getChannelRuntime() {
	cachedChannelRuntimePromise ??= import("./runtime-channel-B_GT8yhH.js").then(({ createRuntimeChannel }) => createRuntimeChannel());
	return cachedChannelRuntimePromise;
}
async function closeMcpLoopbackServerOnDemand() {
	const { closeMcpLoopbackServer } = await import("./mcp-http-BPssmDg-.js");
...
const loadGatewayPluginBootstrapModule = async () => {
	gatewayPluginBootstrapModulePromise ??= import("./server-plugin-bootstrap-DbIjkVsY.js");
...
async function startGatewayServer(port = 18789, opts = {}) {
	...
	const loadStartupPluginsModule = () => {
		startupPluginsModulePromise ??= import("./server-startup-plugins-BewSOGFA.js");
	...
	try {
		...
		const { execApprovalManager, pluginApprovalManager, extraHandlers, coreGatewayHandlers } = await startupTrace.measure("gateway.handlers", async () => {
			const [{ createGatewayAuxHandlers }, { coreGatewayHandlers: coreGatewayHandlersLocal }] = await Promise.all([import("./server-aux-handlers-BEeY9vqa.js"), import("./server-methods-BNtyIGw3.js")]);
			return {
		...
		const [{ attachGatewayWsHandlers }, { listPluginNodeCapabilities }] = await startupTrace.measure("gateway.ws-imports", () => Promise.all([import("./server-ws-runtime-CUNWT1JX.js"), import("./route-capability-Br-EZr5i.js")]));
		...
		const loadScheduledServicesModule = () => {
			scheduledServicesModulePromise ??= import("./server-runtime-services-CjAPXMCQ.js");
		...
		activateScheduledServicesWhenReady();
		const { startManagedGatewayConfigReloader } = await import("./server-reload-handlers-DbyDtFgl.js");
