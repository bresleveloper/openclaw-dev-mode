...
import { t as resolveGatewayAuthToken } from "./auth-token-resolution-EjtyPBok.js";
...
import { i as uiProtocolFreshnessIssueToRepairEffects, r as uiProtocolFreshnessIssueToHealthFinding, t as detectUiProtocolFreshnessIssues } from "./doctor-ui-BNNnkJwC.js";
import { n as collectDisabledCodexPluginRouteIssues } from "./codex-route-warnings-CjRej43m.js";
import { t as getSkippedExecRefStaticError } from "./exec-resolution-policy-BLfFhztF.js";
import { t as detectSkillWorkshopToolPolicyDiagnostic } from "./tool-policy-diagnostic-DrQFARns.js";
...
const loadDoctorWorkspaceModule = async () => await import("./doctor-workspace-8CpVwqDz.js");
...
async function collectSecurityWarningsWithRuntime(cfg) {
	const { collectSecurityWarnings } = await import("./doctor-security-BQgm2f3_.js");
	return collectSecurityWarnings(cfg);
}
async function collectWorkspaceSuggestionNotesWithRuntime(workspaceDir) {
	const { collectWorkspaceBackupTip } = await import("./doctor-state-integrity-CN9oGx7D.js");
...
const hooksModelCheck = {
	...
	async detect(ctx) {
		...
		const { loadModelCatalog } = await import("./model-catalog-B5-rgcvG.js");
		const { getModelRefStatus, resolveConfiguredModelRef, resolveHooksGmailModel } = await import("./model-selection-B-Q3YQMF.js");
...
const legacyStateCheck = {
	...
	source: "doctor",
	async detect(ctx) {
		const { detectLegacyStateMigrations } = await import("./doctor-state-migrations-DnzACdUK.js");
...
const bootstrapSizeCheck = {
	...
	async detect(ctx) {
		...
		const { resolveBootstrapContextForRun } = await import("./bootstrap-files-DxYXpgXa.js");
		const { resolveBootstrapMaxChars, resolveBootstrapTotalMaxChars } = await import("./embedded-agent-helpers-CHIikY9j.js");
...
const legacyWhatsAppCrontabCheck = {
	...
	async detect() {
		const { collectLegacyWhatsAppCrontabHealthWarning } = await import("./cron-BJvUYBoU.js");
...
const legacyCronStoreCheck = {
	...
	defaultEnabled: false,
	async detect(ctx) {
		const { collectLegacyCronStoreHealthFindings } = await import("./cron-BJvUYBoU.js");
...
const gatewayServicesExtraCheck = {
	...
	async detect(ctx) {
		const coreCtx = ctx;
		const { detectExtraGatewayServiceIssues, extraGatewayServiceToHealthFinding } = await import("./doctor-gateway-services-DW9zm5mR.js");
		return (await detectExtraGatewayServiceIssues({ deep: coreCtx.deep === true })).map(extraGatewayServiceToHealthFinding);
	},
	async repair(ctx) {
		const coreCtx = ctx;
		const { detectExtraGatewayServiceIssues, extraGatewayServiceToRepairEffects } = await import("./doctor-gateway-services-DW9zm5mR.js");
...
const gatewayPlatformNotesCheck = {
	...
	source: "doctor",
	async detect(ctx) {
		const { collectMacGatewayPlatformWarnings } = await import("./doctor-platform-notes-CUUrIB9p.js");
