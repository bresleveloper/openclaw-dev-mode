...
const decoders = {
	async oga(buf) {
		...
		if (!decoder) {
			let { OggVorbisDecoder } = await import("./ogg-vorbis-DR82PsPN.js");
	...
	async mp3(buf) {
		...
		if (!decoder) {
			const { MPEGDecoder } = await import("./mpg123-decoder-C6qic7VN.js");
	...
	async flac(buf) {
		...
		if (!decoder) {
			const { FLACDecoder } = await import("./flac-DDbduQch.js");
	...
	async opus(buf) {
		...
		if (!decoder) {
			const { OggOpusDecoder } = await import("./ogg-opus-decoder-3ZHpwhu-.js");
	...
	async wav(buf) {
		...
		if (!decode) {
			let module = await import("./node-wav-lyu5g863.js").then((m) => /* @__PURE__ */ __toESM(m.default, 1));
