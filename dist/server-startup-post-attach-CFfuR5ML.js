...
const loadMainSessionRestartRecoveryModule = async () => {
	mainSessionRestartRecoveryModulePromise ??= import("./main-session-restart-recovery-DPHGdGbJ.js");
...
const loadGatewayRestartSentinelModule = async () => {
	gatewayRestartSentinelModulePromise ??= import("./server-restart-sentinel-DxLeKGSs.js");
...
async function startGatewaySidecars(params) {
	...
	schedulePostReadySidecarTask({
		...
		log: params.log,
		run: async () => {
			const { scheduleSubagentOrphanRecovery } = await import("./subagent-registry-BM4p5DMc.js");
