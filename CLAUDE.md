...
- **Current base**: V2026.7.1 (merged 2026-07-21, branch `upgrade-2026.7.1`). Clean-room patch approach (branch from upstream tag, apply patches fresh). This cycle: whatsapp-kapso-claw plugin fully removed (VPS teardown + fork's socket tap deleted), WA history recorder restored in-fork (`extensions/whatsapp/src/dev-mode/wa-history.ts`), FIX-05 redesigned to daily auto-compact-instead-of-reset, ALL secondary env flags retired (only `OPENCLAW_DEV_MODE=1` gates anything), SEC-97 UI half deferred (upstream deleted the whole `ui/src/ui/` tree and rebuilt the Control UI as a page router). See "V2026.7.1 Upgrade" below.
